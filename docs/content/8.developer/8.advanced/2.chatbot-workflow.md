...
To support smaller local models (e.g. Ollama running Gemma or Llama models) that have limited context windows and struggle with high tool counts, the chatbot does not expose all available tools at once. Instead, it utilizes **Dynamic Tool Selection** and **Auto Intent Classification**.
...
*   **`full` Profile (35+ tools total):** Contains all developer, coaching, and granular utility tools.
*   **`core` Profile (20 tools total):** Contains only the essential logging and tracking tools for food, exercise, checkins, and goals.

### 2. Auto Intent Classification Pipeline
If the user does not manually override tool categories in the chat interface dropdown, the backend dynamically calculates the minimum required tool categories for each message via an automatic classification pipeline:

```mermaid
graph TD
    A[Incoming User Message] --> B[Phase 1: Keyword Regex Matching]
    B -- Match Found --> C[Load Specific Category Tools e.g., food only]
    B -- No Match --> D[Phase 2: LLM Intent Classifier]
    D -- Success --> E[Load Classified Category Tools]
    D -- Fail / 'none' --> F[Phase 3: Safe Default Core Categories]
    F --> G[Load 20 Core Tools]
```

#### Phase 1: Fast Keyword Regex Matching (0ms Overhead)
Before making any model calls, the server matches the user's message against a set of predefined regex keywords for each domain (e.g. `"calories"` or `"eat"` for the `food` domain, `"walked"` or `"lifted"` for `exercise`). 
*   **Outcome:** If a match is found, the server bypasses the LLM classifier completely. This results in **0ms overhead** and **0 token cost**.

#### Phase 2: LLM Classifier Fallback (Low-Latency)
If the keyword matcher is unsure, it triggers a low-latency LLM intent classification request.
...
#### Phase 3: Safe Baseline Fallback
If the LLM classifier returns `"none"` or fails due to network/service errors:
*   **Outcome:** The server falls back to loading the default core categories `['food', 'exercise', 'checkin', 'goals']` which loads **20 tools**. This ensures the user is never blocked from performing standard operations.
...
### 4. Resilient Tool Validation Fallbacks
