...
*   **By design, this list is intentionally not exhaustive.** User phrasing is unbounded across paraphrases, typos, and languages; growing the regex list indefinitely trades recall for false positives (e.g. an overly broad "run" rule would fire on "I ran out of milk"). Gaps here are expected to be caught by Phase 2, or recovered from via the escalation tool (below) if even Phase 2 misses.
