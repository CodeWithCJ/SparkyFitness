...
| `openfoodfacts_sync_queue` | Durable, revision-aware automatic upload state with retained success and failure history |
...
Open Food Facts automatic sync requires both the server-wide
`allow_openfoodfacts_contributions` gate and the user's
`auto_contribute_openfoodfacts` opt-in. Food and default-variant triggers upsert one
`openfoodfacts_sync_queue` row per eligible custom product and increment its revision
whenever uploadable data changes. Eligibility excludes products sourced from Open
Food Facts or proprietary providers, requires a non-empty name and checksum-valid
GTIN, and only permits metric-convertible default variants.

Workers claim at most one due row with `FOR UPDATE SKIP LOCKED`, use
revision-aware completion/retry updates, and resolve the currently eligible personal
account or administrator-approved global fallback at delivery time. Successful rows
remain available as sync history. Product Opener reads from both interactive barcode
lookups and automatic contribution preparation acquire the same database-coordinated
permit immediately before the request. The permit is held through the response and
enforces at least five seconds between product reads across server instances; search
endpoints are not throttled by this limiter. A busy automatic job is deferred without
spending its upload-attempt budget. This keeps food writes independent of external
network availability and prevents duplicate queue claims or clustered product reads.
