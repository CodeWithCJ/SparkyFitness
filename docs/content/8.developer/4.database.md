...
```sql
-- Example RLS policy for diary logs (writable by delegates with can_manage_diary)
ALTER TABLE food_entries ENABLE ROW LEVEL SECURITY;
CREATE POLICY food_entries_diary_access ON food_entries
  FOR ALL USING (
    user_id = ANY(ARRAY[
      public.get_user_id(),
...
**Key functions:**
- `public.get_user_id()` — active profile context (may differ during family delegation)
- `public.get_authenticated_user_id()` — the true logged-in actor (never changes)
- `public.get_permission(userId, authenticatedUserId, permissionType)` — checks diary/checkin/medications/cycle/reports access

See `SparkyFitnessServer/db/rls_policies.sql` for the complete policy set.
...
2. **Write the migration SQL** following the checklist in [`../../agent-docs/new-migration-checklist.md`](../../agent-docs/new-migration-checklist.md)
