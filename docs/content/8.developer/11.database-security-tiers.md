...
1. **`public.get_user_id()`**: Returns the ID of the **active profile context** being viewed (which changes during family delegation context switching).
2. **`public.get_authenticated_user_id()`**: Returns the ID of the **true authenticated logged-in actor** (which never changes during context switching).
3. **`public.get_permission(userId, authenticatedUserId, permissionType)`**: Checks if the authenticated user has the specified permission over the user context (diary, checkin, medications, cycle, reports).
