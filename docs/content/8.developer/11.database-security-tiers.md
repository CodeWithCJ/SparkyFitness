...
| `food_entry_meals` | Meal details associated with logged food entries, plus an optional per-entry markdown `notes` field | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
