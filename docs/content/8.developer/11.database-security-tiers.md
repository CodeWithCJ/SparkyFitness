...
| `exercise_entry_gps_points` | GPS trackpoints & telemetry for exercise entries | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `heart_rate_entries` | Intraday continuous heart rate telemetry | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
| `hrv_entries` | Intraday & overnight HRV telemetry | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
| `respiration_entries` | Intraday breathing rate telemetry | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
| `spo2_entries` | Pulse ox SpO2 percentage telemetry | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
