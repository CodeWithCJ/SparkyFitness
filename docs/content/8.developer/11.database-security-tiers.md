...
PostgreSQL session variables are initialized at the start of every pool client transaction (via the `public.set_app_context(userId, authenticatedUserId)` helper). RLS policies evaluate access using two main user ID functions:

1. **`current_user_id()`**: Returns the ID of the **active profile context** being viewed (which changes during family delegation context switching).
2. **`authenticated_user_id()`**: Returns the ID of the **true authenticated logged-in actor** (which never changes during context switching).
...
| `user_medication_display_preferences` | Private preference on GLP-1/Medication display | Owner-Only | Owner-Only |
...
| `user_preferences` | Unit display preferences (Metric vs Imperial) | **Owner-Only** | Switched Delegate |
...
| `meal_types` | Custom breakfast, lunch, dinner, snack layouts | **Owner-Only** | Switched Delegate |
| `foods` | Custom food items created by the user | **Owner-Only** | Delegate with `can_view_food_library` |
| `food_variants` | Serving options / sizes for custom foods | **Owner-Only** | Delegate with `can_view_food_library` |
| `meals` | Custom meals created by the user | **Owner-Only** | Delegate with `can_view_food_library` |
| `meal_foods` | Food items assigned inside a custom meal | **Owner-Only** | Delegate with `can_view_food_library` |
| `exercises` | Custom exercises created by the user | **Owner-Only** | Delegate with `can_view_exercise_library` |
| `workout_presets` | Custom workouts/presets created by the user | **Owner-Only** | Delegate with `can_view_exercise_library` |
| `workout_preset_exercises` | Exercises assigned inside a workout preset | **Owner-Only** | Delegate with `can_view_exercise_library` |
| `workout_preset_exercise_sets` | Reps/sets configured inside a preset exercise | **Owner-Only** | Delegate with `can_view_exercise_library` |
| `workout_plan_templates` | Templates for weekly workout schedules | **Owner-Only** | Delegate with `can_view_exercise_library` |
| `medications` | Custom medication inventory lists (e.g. insulin) | **Owner-Only** | Delegate with `can_manage_medications` |
