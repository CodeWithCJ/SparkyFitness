...
| `foods` | Custom food items created by the user, including their `images` (jsonb array of upload paths / provider URLs) | **Owner-Only** | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
...
| `meals` | Custom meals created by the user, including their `images` (jsonb array of upload paths / provider URLs) | **Owner-Only** | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
...
| Table Name | Description | Write (insert/update/delete) | Read (select) |
| :--- | :--- | :--- | :--- |
| `food_entries` | Logged meals/calories consumed for the day. `image_url` is a per-entry override photo and is never written back to the parent `foods`/`meals` row | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
