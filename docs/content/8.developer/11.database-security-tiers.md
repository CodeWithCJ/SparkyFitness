...
| `meal_foods` | Food items assigned inside a custom meal | **Owner-Only** | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
...
| `medications` | Custom medication inventory lists (e.g. insulin) | **Owner-Only** | Delegate with `can_manage_medications` or `can_view_reports` |
