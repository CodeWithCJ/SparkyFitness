...
*   **OIDC (env-configured provider)**  
    ...
    *   **`SPARKY_FITNESS_OIDC_ADMIN_GROUP`**, **`SPARKY_FITNESS_OIDC_USER_GROUP`**: Group/claim values from your IdP for role mapping. Configure your IdP to send these in token claims; `ADMIN_GROUP` is used to automatically grant admin privileges.
