...
*   **`ALLOW_PRIVATE_NETWORK_CORS`**: Set to `true` to allow Cross-Origin Resource Sharing (CORS) from private network addresses (e.g., `192.168.x.x`, `10.x.x.x`, `127.0.0.1`, `localhost`). This is useful for self-hosted environments where you access the app via a local IP rather than a public domain. Use with caution as it relaxes security.
