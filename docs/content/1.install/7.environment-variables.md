...
- **`SPARKY_FITNESS_DISABLE_UPSTREAM_NOTICES`**: Set to `true` to stop the server from contacting github.com for the two notice features: fetching `announcement.md` from the `main` branch (re-fetched at most every 15 minutes) and querying the releases API for a newer version. A local `announcement.md`, if you ship one, is still served, and the app keeps reporting its own version. Defaults to `false`.
