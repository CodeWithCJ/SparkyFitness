...
### Demo Mode Configuration (Server Side)

Configure these variables to enable an isolated, auto-seeded demo account for public demonstrations or evaluation instances.

- **`SPARKY_FITNESS_DEMO_MODE`**: Set to `true` to enable Demo Mode. When enabled:
  - The server automatically seeds a realistic demo account with exercises, workouts, meals, biometric logs, check-ins, and fasting history using relative dates.
  - Adds a 1-click passwordless "🚀 Explore Demo" button on the login page with an interactive evaluation & anti-abuse disclaimer dialog.
  - Automatically resets the demo account's data daily at midnight UTC (`0 0 * * *`) strictly scoped to the demo account ID without affecting any real user data.
  - Protects the demo account from modification. A single deny-list runs ahead of the whole route table and blocks, for the demo account only: all account management under `/api/identity` (password, email, 2FA/TOTP, passkeys, API keys, family sharing, profile), the equivalent Better Auth endpoints, every file upload on any route, AI features that bill the operator's LLM provider (`/api/chat`, `/api/ai`, `/mcp`), third-party integration binding (`/api/integrations`, `/api/withings`, `/api/external-providers`), and the admin surface.
  - `/api/auth/demo-login` is rate limited per source address, and the server refuses to start if `SPARKY_FITNESS_ADMIN_EMAIL` matches the demo account.
  - Automatically purges and removes the demo user and all its associated data if the server is subsequently started with `SPARKY_FITNESS_DEMO_MODE=false`.
  - Defaults to `false`.
- **`SPARKY_FITNESS_DEMO_EMAIL`**: The email address for the demo user. Defaults to `demo@sparkyfitness.com`. **Must not be an address a real account already uses** — the demo account is identified by a marker written into its profile, and seeding, the daily reset, and the auto-purge all refuse to touch an account that does not carry it.
- **`SPARKY_FITNESS_DEMO_PASSWORD`**: The internal password assigned to the demo account. (Can also be supplied via **`SPARKY_FITNESS_DEMO_PASSWORD_FILE`**). If not set, a cryptographically secure random password is automatically generated in memory on startup.
...
| `SPARKY_FITNESS_DEMO_PASSWORD` | `SPARKY_FITNESS_DEMO_PASSWORD_FILE` | Demo user account password (when Demo Mode is enabled) |
