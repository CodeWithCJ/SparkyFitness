...
- **`SPARKY_FITNESS_DEMO_MODE`**: Set to `true` to enable Demo Mode. When enabled:
  ...
  - Protects the demo account from modification (blocks password changes, 2FA/TOTP/passkey enrollment, email changes, external OAuth provider binding, and account deletion).
  ...
  - Defaults to `false`.
- **`SPARKY_FITNESS_DEMO_EMAIL`**: The email address for the demo user. Defaults to `demo@sparkyfitness.com`.
