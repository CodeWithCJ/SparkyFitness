...
These optional server-side variables enable the instance-wide HUAWEI Health
Service Kit Web integration. Without the required pair, the web UI remains in
a safe unavailable state.

- **`SPARKY_FITNESS_HUAWEI_HEALTH_CLIENT_ID`**: Huawei Web application OAuth client id.
- **`SPARKY_FITNESS_HUAWEI_HEALTH_CLIENT_SECRET`**: Huawei OAuth client secret. Keep it server-side and never expose it through a `VITE_*` variable.
- **`SPARKY_FITNESS_HUAWEI_HEALTH_APP_ID`**: Huawei application id used by Health consent endpoints. Optional when it is identical to the client id.
- **`SPARKY_FITNESS_HUAWEI_HEALTH_REDIRECT_URI`**: Exact registered OAuth callback. Optional; defaults to `${SPARKY_FITNESS_FRONTEND_URL}/huaweihealth/callback`.

Huawei Health Service scope verification must be approved before general user
availability. The server also requires a stable
`SPARKY_FITNESS_API_ENCRYPTION_KEY` because access and refresh tokens are stored
encrypted. See the [HUAWEI Health integration guide](/features/settings/huawei-health).
