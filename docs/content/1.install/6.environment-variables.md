...
If you are running the optional Model Context Protocol (MCP) server to connect AI assistants to your health data, the following variables are relevant. The MCP server shares the core database variables with the main server.

- **`SPARKY_FITNESS_API_KEY`**: A personal API key used to authenticate the AI client (like Cursor or Claude Desktop) against your SparkyFitness instance. You must generate this key inside your SparkyFitness user settings, and then provide it to your AI client so it can securely access your data.
- **`MCP_TRANSPORT`**: The transport protocol used by the MCP server. Defaults to `http` for Docker deployments (where the AI client talks to the server over an HTTP stream), but can also be set to `stdio` for local binary execution.
- **`DEV_TOOLS_ENABLED`**: (Optional) Set to `true` to enable deep database inspection tools for the AI. **Requires the authenticated user to be an admin.** Do not enable this in production unless you are actively debugging schema issues.
