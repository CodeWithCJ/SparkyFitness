...
- **Local dev**: `http://localhost:3010/mcp` — hit the **server** port directly. The frontend Vite dev proxy does **not** forward `/mcp`, so pointing an MCP client at the frontend port will not work.
...
_Note: for a local-dev server over plain HTTP, add `--allow-http` to the args and use `http://localhost:3010/mcp` — `mcp-remote` refuses non-HTTPS URLs otherwise._
