...
| :--- | :--- | :--- |
| **Manage Diary**<br>`can_manage_diary` | ✍️ Food entries, exercise entries, water logs, daily goal plans, favorite (starred) foods/meals (logged meals snapshot their serving size and yield at log time, so delegates and owners see the exact portion logged even if the master recipe changes). | 📖 Food entries, exercise entries, water logs, daily goal plans, favorite (starred) foods/meals, custom food/exercise libraries, and read-only profile/layout/water-container settings.<br><br>❌ *Wellness logs (fasting, sleep, mood) and medication logs are completely blocked.* |
| **Manage Check-in**<br>`can_manage_checkin` | ✍️ Weight, body measurements, progress photos, mood logs, fasting status, sleep logs, smart scale composition (muscle mass, bone mass, water %, BMR), and generic health telemetry (heart rate, HRV, respiration, SpO2, vitals, daily health scores, and captured wearable calorie totals). | 📖 Weight history, progress photos, wellness logs (mood, fasting, sleep), smart scale metrics, generic health telemetry, and read-only profile/layout settings.<br><br>❌ *Diary logs (food, exercise, water) and medication logs are completely blocked.* |
...
| **Share Integrations**<br>`share_external_providers` | ❌ **No write privileges** (cannot add, edit, or delete provider configurations — owner-only). | 📖 Allows the family & friends to use the owner's non-private search provider configurations (e.g., FatSecret, USDA, OpenFoodFacts) to search for food and exercise items and add them to their own library.<br><br>❌ *Shared personal provider credentials are never used for Open Food Facts contributions. Automatic uploads require both the server-wide administrator switch and the food owner's own opt-in; this sharing permission grants neither consent. Their automatic product reads also share one server-wide safety budget rather than a delegate-specific allowance.*<br><br>❌ *Health integrations (Garmin, Fitbit, Withings, Google Health, Polar, Strava, Hevy) are strictly private and **never** shared — this permission does not grant access to them.* |
