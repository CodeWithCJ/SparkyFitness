...
| **Manage Check-in**<br>`can_manage_checkin` | ✍️ Weight, body measurements, progress photos, mood logs, fasting status, sleep logs. | 📖 Weight history, progress photos, wellness logs (mood, fasting, sleep), and read-only profile/layout settings.<br><br>❌ *Diary logs (food, exercise, water) and medication logs are completely blocked.* |
