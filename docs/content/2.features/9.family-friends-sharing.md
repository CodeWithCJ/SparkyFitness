...
| **Manage Check-in**<br>`can_manage_checkin` | ✍️ Weight, body measurements, progress photos, mood logs, fasting status, sleep logs, smart scale composition (muscle mass, bone mass, water %, BMI), and generic health telemetry (heart rate, HRV, respiration, SpO2, vitals, daily health scores). | 📖 Weight history, progress photos, wellness logs (mood, fasting, sleep), smart scale metrics, generic health telemetry, and read-only profile/layout settings.<br><br>❌ *Diary logs (food, exercise, water) and medication logs are completely blocked.* |
