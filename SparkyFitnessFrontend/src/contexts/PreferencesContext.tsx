...
import { createWaterContainer, setPrimaryWaterContainer } from '@/services/waterContainerService';
import {
...
interface PreferencesContextType {
  ...
  convertDistance: (value: number, from: 'km' | 'miles', to: 'km' | 'miles') => number;
  convertEnergy: (value: number, fromUnit: EnergyUnit, toUnit: EnergyUnit) => number;
  ...
  saveAllPreferences: (newPrefs?: Partial<PreferencesContextType>) => Promise<void>;
}
...
export const PreferencesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [autoClearHistory, setAutoClearHistoryState] = useState<string>('never');
  const [loggingLevel, setLoggingLevelState] = useState<'DEBUG' | 'INFO' | 'WARN' | 'ERROR' | 'SILENT'>('ERROR');
  const [defaultFoodDataProviderId, setDefaultFoodDataProviderIdState] = useState<string | null>(null);
  ...
  const [calorieGoalAdjustmentMode, setCalorieGoalAdjustmentModeState] = useState<'dynamic' | 'fixed'>('dynamic');
  const [energyUnit, setEnergyUnitState] = useState<EnergyUnit>('kcal');
  const [autoScaleOpenFoodFactsImports, setAutoScaleOpenFoodFactsImportsState] = useState<boolean>(false);
  ...
  const formatDateInUserTimezone = useCallback((date: string | Date, formatStr?: string) => {
  ...
  const parseDateInUserTimezone = useCallback((dateString: string): Date => {
  ...
  const updatePreferences = useCallback(async (updates: Partial<{
    ...
    if (!user) {
      if (updates.default_weight_unit) localStorage.setItem('weightUnit', updates.default_weight_unit);
      if (updates.default_measurement_unit) localStorage.setItem('measurementUnit', updates.default_measurement_unit);
      if (updates.default_distance_unit) localStorage.setItem('distanceUnit', updates.default_distance_unit);
      if (updates.date_format) localStorage.setItem('dateFormat', updates.date_format);
      if (updates.language) localStorage.setItem('language', updates.language);
      if (updates.calorie_goal_adjustment_mode) localStorage.setItem('calorieGoalAdjustmentMode', updates.calorie_goal_adjustment_mode);
      if (updates.energy_unit) localStorage.setItem('energyUnit', updates.energy_unit);
  ...
  const saveAllPreferences = useCallback(async (newPrefs?: Partial<PreferencesContextType>) => {
    ...
    const prefsToSave = {
      ...
      calorie_goal_adjustment_mode: newPrefs?.calorieGoalAdjustmentMode ?? calorieGoalAdjustmentMode,
      energy_unit: newPrefs?.energyUnit ?? energyUnit,
      auto_scale_open_food_facts_imports: newPrefs?.autoScaleOpenFoodFactsImports ?? autoScaleOpenFoodFactsImports,
    ...
    loggingLevel, weightUnit, measurementUnit, distanceUnit, dateFormat, autoClearHistory,
    defaultFoodDataProviderId, timezone, itemDisplayLimit, foodDisplayLimit, waterDisplayUnit,
    language, calorieGoalAdjustmentMode, energyUnit, autoScaleOpenFoodFactsImports, bmrAlgorithm,
    bodyFatAlgorithm, includeBmrInNetCalories, fatBreakdownAlgorithm, mineralCalculationAlgorithm,
    vitaminCalculationAlgorithm, sugarCalculationAlgorithm, selectedDiet, updatePreferences
  ]);
  ...
  const createDefaultPreferences = useCallback(async () => {
    ...
    try {
      const defaultPrefs = {
        ...
        system_prompt: 'You are Sparky, a helpful AI assistant for health and fitness tracking.',
  ...
  const loadPreferences = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setDefaultFoodDataProviderIdState(data.default_food_data_provider_id || null);
        setTimezoneState(data.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone);
        ...
        setCalorieGoalAdjustmentModeState(data.calorie_goal_adjustment_mode || 'dynamic');
        setEnergyUnitState(data.energy_unit || 'kcal');
        setAutoScaleOpenFoodFactsImportsState(data.auto_scale_open_food_facts_imports ?? false);
        setBmrAlgorithmState(data.bmr_algorithm || BmrAlgorithm.MIFFLIN_ST_JEOR);
        setBodyFatAlgorithmState(data.body_fat_algorithm || BodyFatAlgorithm.US_NAVY);
        setIncludeBmrInNetCaloriesState(data.include_bmr_in_net_calories ?? false);
        setFatBreakdownAlgorithmState(data.fat_breakdown_algorithm || FatBreakdownAlgorithm.AHA_GUIDELINES);
        setMineralCalculationAlgorithmState(data.mineral_calculation_algorithm || MineralCalculationAlgorithm.RDA_STANDARD);
        setVitaminCalculationAlgorithmState(data.vitamin_calculation_algorithm || VitaminCalculationAlgorithm.RDA_STANDARD);
        setSugarCalculationAlgorithmState(data.sugar_calculation_algorithm || SugarCalculationAlgorithm.WHO_GUIDELINES);
  ...
  }, [user, loggingLevel, createDefaultPreferences, createDefaultWaterContainer]);
  ...
  const setLoggingLevel = useCallback((level: 'DEBUG' | 'INFO' | 'WARN' | 'ERROR' | 'SILENT') => {
    setLoggingLevelState(level);
  }, []);

  const setCalorieGoalAdjustmentMode = useCallback((mode: 'dynamic' | 'fixed') => {
  ...
  const setAutoScaleOpenFoodFactsImports = useCallback((enabled: boolean) => {
  ...
  useEffect(() => {
    if (!loading) {
      if (user) {
        ...
        const savedDistanceUnit = localStorage.getItem('distanceUnit') as 'km' | 'miles';
        const savedDateFormat = localStorage.getItem('dateFormat');
        const savedLanguage = localStorage.getItem('language');
        const savedCalorieGoalAdjustmentMode = localStorage.getItem('calorieGoalAdjustmentMode') as 'dynamic' | 'fixed';
        const savedEnergyUnit = localStorage.getItem('energyUnit') as EnergyUnit;
        const savedAutoScaleOpenFoodFactsImports = localStorage.getItem('autoScaleOpenFoodFactsImports');
        ...
        if (savedCalorieGoalAdjustmentMode) setCalorieGoalAdjustmentModeState(savedCalorieGoalAdjustmentMode);
        if (savedEnergyUnit) setEnergyUnitState(savedEnergyUnit);
        if (savedAutoScaleOpenFoodFactsImports !== null) setAutoScaleOpenFoodFactsImportsState(savedAutoScaleOpenFoodFactsImports === 'true');
      }
    }
  ...
  const contextValue = useMemo(() => ({
    ...
    weightUnit, measurementUnit, distanceUnit, dateFormat, autoClearHistory,
    loggingLevel, defaultFoodDataProviderId, timezone, itemDisplayLimit, foodDisplayLimit,
    calorieGoalAdjustmentMode, energyUnit, autoScaleOpenFoodFactsImports, nutrientDisplayPreferences,
    waterDisplayUnit, language, bmrAlgorithm, bodyFatAlgorithm, includeBmrInNetCalories,
    fatBreakdownAlgorithm, mineralCalculationAlgorithm, vitaminCalculationAlgorithm,
    sugarCalculationAlgorithm, selectedDiet, setWeightUnit, setMeasurementUnit, setDistanceUnit,
    setDateFormat, setAutoClearHistory, setLoggingLevel, setDefaultFoodDataProviderId,
    setTimezone, setItemDisplayLimit, setCalorieGoalAdjustmentMode, setEnergyUnit,
    setAutoScaleOpenFoodFactsImports, loadNutrientDisplayPreferences, convertWeight,
    convertMeasurement, convertDistance, convertEnergy, getEnergyUnitString, formatDate,
    formatDateInUserTimezone, parseDateInUserTimezone, loadPreferences, saveAllPreferences
  ]);
