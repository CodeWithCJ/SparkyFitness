...
export const ActiveUserProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    if (!loading) {
      // Only proceed after authentication loading is complete
      if (user) {
        const currentActiveUserId = user.activeUserId || user.id;

        info(
          loggingLevel,
          'ActiveUserProvider: User logged in, setting active user context.'
        );
        ...
        info(
          loggingLevel,
          'ActiveUserProvider: User logged out, clearing active user and accessible users.'
        );
  ...
  const loadAccessibleUsers = useCallback(
    async (initialActiveUserId?: string) => {
      if (!user) {
        warn(
          loggingLevel,
          'ActiveUserProvider: Attempted to load accessible users without a user.'
        );
        return;
      }
      info(
        loggingLevel,
        'ActiveUserProvider: Loading accessible users for user:',
        user.id
      );
      ...
      try {
        ...
        info(
          loggingLevel,
          'ActiveUserProvider: Accessible users data received:',
          data
        );
        ...
        error(
          loggingLevel,
          'ActiveUserProvider: Unexpected error loading accessible users:',
          err
        );
      }
    },
  ...
  useEffect(() => {
    if (user && user.activeUserId && user.activeUserId !== activeUserId) {
      debug(
        loggingLevel,
        'ActiveUserProvider: Detected activeUserId change in auth context, updating local state from:',
        activeUserId,
        'to:',
        user.activeUserId
      );
  ...
  useEffect(() => {
    ...
    info(
      loggingLevel,
      'ActiveUserProvider: Updating active user name for activeUserId:',
      activeUserId
    );
    ...
    if (!activeUserId || activeUserId === user.id) {
      info(
        loggingLevel,
        'ActiveUserProvider: Acting on own behalf, name:',
        user.fullName || user.email
      );
      setActiveUserName(user.fullName || user.email || 'You');
    } else {
      // If acting on behalf of someone else, look up their name from accessible users
      const activeUser = accessibleUsers.find(
        (u) => u.user_id === activeUserId
      );
      if (activeUser) {
        info(
          loggingLevel,
          'ActiveUserProvider: Found family member name:',
          activeUser.full_name || activeUser.email
        );
        setActiveUserName(
          activeUser.full_name || activeUser.email || 'Family Member'
        );
      } else {
        // If not found, show a generic name
        info(
          loggingLevel,
          'ActiveUserProvider: Active user not found in accessible users, showing generic name'
        );
  ...
  const switchToUser = async (userId: string | null) => {
    if (!user) {
      warn(
        loggingLevel,
        'ActiveUserProvider: Attempted to switch user without a logged-in user.'
      );
      return;
    }
    info(
      loggingLevel,
      'ActiveUserProvider: Attempting to switch active user from:',
      activeUserId,
      'to:',
      userId
    );
    ...
    try {
      ...
      info(
        loggingLevel,
        'ActiveUserProvider: Profile switch successful. New activeUserId should be:',
        targetUserId
      );
    } catch (err) {
      error(
        loggingLevel,
        'ActiveUserProvider: Failed to switch profile context:',
        err
      );
      throw err;
    }
  ...
  const hasPermission = (permission: string): boolean => {
    debug(
      loggingLevel,
      'ActiveUserProvider: Checking permission:',
      permission,
      'for active user:',
      activeUserId
    );
    if (!user || !activeUserId) {
      debug(
        loggingLevel,
        'ActiveUserProvider: No user or activeUserId, returning false for permission:',
        permission
      );
      return false;
    }
    ...
    if (activeUserId === user.id) {
      debug(
        loggingLevel,
        'ActiveUserProvider: User is acting on own behalf, granting permission:',
        permission
      );
      return true;
    }
    ...
    if (!accessibleUser) {
      warn(
        loggingLevel,
        'ActiveUserProvider: Accessible user not found for activeUserId:',
        activeUserId
      );
      return false;
    }
    ...
    if (directPermission) {
      debug(
        loggingLevel,
        'ActiveUserProvider: Direct permission granted for:',
        permission
      );
      return true;
    }
    ...
    if (accessibleUser.permissions.reports) {
      if (permission === 'diary' || permission === 'checkin') {
        debug(
          loggingLevel,
          'ActiveUserProvider: Read-only access inherited from reports for:',
          permission
        );
  ...
  const hasWritePermission = (permission: string): boolean => {
    debug(
      loggingLevel,
      'ActiveUserProvider: Checking write permission:',
      permission,
      'for active user:',
      activeUserId
    );
    if (!user || !activeUserId) {
      debug(
        loggingLevel,
        'ActiveUserProvider: No user or activeUserId, returning false for write permission:',
        permission
      );
      return false;
    }
    ...
    if (activeUserId === user.id) {
      debug(
        loggingLevel,
        'ActiveUserProvider: User is acting on own behalf, granting write permission:',
        permission
      );
      return true;
    }
    ...
    debug(
      loggingLevel,
      'ActiveUserProvider: Direct write permission for',
      permission,
      'is:',
      granted
    );
  ...
  debug(
    loggingLevel,
    'ActiveUserProvider: isActingOnBehalf:',
    isActingOnBehalf
  );
