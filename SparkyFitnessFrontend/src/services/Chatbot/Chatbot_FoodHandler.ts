...
export const processFoodInput = async (
  ...
  try {
    debug(
      userLoggingLevel,
      `[${transactionId}] Processing food input with data:`,
      data,
      'and entryDate:',
      entryDate
    );
    ...
    const meal_type =
      raw_meal_type?.toLowerCase() === 'snack' ? 'snacks' : raw_meal_type;
    ...
    debug(
      userLoggingLevel,
      'No food options array received, searching database for:',
      food_name
    );
    ...
    let exactFoods = null;
    try {
      exactFoods = await apiCall(
        `/foods/search?name=${encodeURIComponent(food_name)}&exactMatch=true`,
        {
          method: 'GET',
        }
      );
    ...
    if (!existingFoods || existingFoods.length === 0) {
      debug(
        userLoggingLevel,
        'No exact match found, searching broadly for:',
        food_name
      );
      let broadFoods = null;
      try {
        broadFoods = await apiCall(
          `/foods/search?name=${encodeURIComponent(food_name)}&broadMatch=true`,
          {
            method: 'GET',
          }
        );
    ...
    if (existingFoods && existingFoods.length > 0) {
      ...
      if (
        ...
        unit.toLowerCase() !== food.default_variant.serving_unit.toLowerCase()
      ) {
        warn(
          userLoggingLevel,
          `Unit mismatch: User requested '${quantity}${unit}' but database serving unit is '${food.default_variant.serving_size}${food.default_variant.serving_unit}'. Triggering AI options.`
        );
        return {
      ...
      try {
        ...
        return {
          action: 'none',
          response:
            "Sorry, I couldn't add that to your diary. Please try again.",
        };
      }
      ...
      const calories = Math.round(
        (food.default_variant.calories || 0) *
          (quantity / (food.default_variant.serving_size || 100))
      );
      ...
      info(
        userLoggingLevel,
        'Food not found in database. Returning fallback data.'
      );
    ...
    error(
      userLoggingLevel,
      '❌ [Nutrition Coach] Error processing food input:',
      err
    );
    ...
    return {
      action: 'none',
      response:
        'Sorry, I had trouble processing that. Could you try rephrasing what you ate?',
    };
  }
};
...
export const addFoodOption = async (
  ...
  try {
    ...
    try {
      existingFoods = await apiCall(
        `/foods/search?name=${encodeURIComponent(selectedOption.name)}&exactMatch=true&checkCustom=true`,
        {
          method: 'GET',
        }
      );
      ...
      return {
        action: 'none',
        response:
          'Sorry, I had trouble checking for existing food. Please try again.',
      };
    }

    const foodToUse =
      existingFoods && existingFoods.length > 0 ? existingFoods[0] : null;

    if (foodToUse) {
      foodId = foodToUse.id;
      // Check if the selected option's unit is the same as the base food's unit
      if (
        selectedOption.serving_unit.toLowerCase() ===
        foodToUse.serving_unit.toLowerCase()
      ) {
        info(
          userLoggingLevel,
          `[${transactionId}] Selected option unit matches base food unit. Logging as base food.`
        );
        ...
        try {
          existingVariant = await apiCall(
            `/food-variants?food_id=${foodId}&serving_unit=${encodeURIComponent(selectedOption.serving_unit)}`,
            {
              method: 'GET',
            }
          );
          ...
          return {
            action: 'none',
            response:
              'Sorry, I had trouble checking for existing food variants. Please try again.',
          };
        }
        ...
        if (existingVariant && existingVariant.length > 0) {
          ...
          info(
            userLoggingLevel,
            `[${transactionId}] Existing food variant found:`,
            variantId
          );
        } else {
          ...
          info(
            userLoggingLevel,
            `[${transactionId}] Creating new food variant for existing food:`,
            selectedOption.name
          );
          let newVariant = null;
          try {
            ...
            return {
              action: 'none',
              response:
                "Sorry, I couldn't create that food variant. Please try again.",
            };
          }
      ...
      info(
        userLoggingLevel,
        `[${transactionId}] Creating new food entry:`,
        selectedOption.name
      );
      let newFood = null;
      try {
    ...
    debug(
      userLoggingLevel,
      `[${transactionId}] Preparing to insert food entry. Selected Option:`,
      selectedOption
    );
    ...
    try {
      ...
      return {
        action: 'none',
        response:
          "I created the food/variant but couldn't add it to your diary. Please try again.",
      };
    }
    info(
      userLoggingLevel,
      `[${transactionId}] Food entry inserted successfully.`
    );
    ...
    return {
      action: 'none',
      response:
        'Sorry, I encountered an error adding that food. Please try again.',
    };
  }
};
