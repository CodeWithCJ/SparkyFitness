...
export const customNutrientService = {
    async createCustomNutrient(nutrientData: Omit<UserCustomNutrient, 'id' | 'user_id' | 'created_at' | 'updated_at'>): Promise<UserCustomNutrient> {
        const response = await api.post('/custom-nutrients', { body: nutrientData });
        return response;
    },
    ...
    async updateCustomNutrient(id: string, nutrientData: Partial<Omit<UserCustomNutrient, 'id' | 'user_id' | 'created_at' | 'updated_at'>>): Promise<UserCustomNutrient> {
        const response = await api.put(`/custom-nutrients/${id}`, { body: nutrientData });
        return response.data;
    },
