...
export const startFast = async (startTime: Date, targetEndTime: Date, fastingType: string): Promise<FastingLog> => {
...
export const getFastingHistory = async (limit: number = 20, offset: number = 0): Promise<FastingLog[]> => {
...
export const getFastingDataRange = async (startDate: string, endDate: string): Promise<FastingLog[]> => {
    const response = await api.get(`/fasting/history/range/${startDate}/${endDate}`);
    return response || [];
};

export const updateFast = async (id: string, updates: Partial<FastingLog>): Promise<FastingLog> => {
