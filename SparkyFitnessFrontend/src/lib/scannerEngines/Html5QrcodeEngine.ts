...
export class Html5QrcodeEngine implements BarcodeScannerEngine {
    ...
    init(element: HTMLElement) {
        ...
        if (!this.containerElement.id) {
            this.containerElement.id = 'html5-qrcode-reader-' + Math.random().toString(36).substr(2, 9);
        }
    }
    ...
    async start(constraints: MediaStreamConstraints, deviceId?: string): Promise<void> {
        ...
        if (deviceId) {
            ...
            cameraIdOrConfig = (facingMode === 'user') ? { facingMode: 'user' } : { facingMode: 'environment' };
        }
