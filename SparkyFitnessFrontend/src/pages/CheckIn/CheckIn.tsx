...
const CheckIn = () => {
  ...
  const [useMostRecentForCalculation, setUseMostRecentForCalculation] =
    useState(false); // New state for checkbox
  ...
  useEffect(() => {
    ...
    const handleRefresh = () => {
      info(
        loggingLevel,
        'CheckIn: Received measurementsRefresh event, triggering data reload.'
      );
      loadExistingData();
  ...
  useEffect(() => {
    info(
      loggingLevel,
      `CheckIn: useEffect for selectedDate triggered. currentUserId: ${currentUserId}, customCategories.length: ${customCategories.length}, selectedDate: ${selectedDate}`
    );
    if (currentUserId) {
      // Removed customCategories.length > 0 condition
      loadExistingData();
      fetchAllRecentMeasurements();
    }
  }, [
  ...
  const loadCustomCategories = async () => {
    if (!currentUserId) {
      warn(
        loggingLevel,
        'CheckIn: loadCustomCategories called with no current user ID.'
      );
      return;
    }
  ...
  const fetchAllRecentMeasurements = async () => {
    if (!currentUserId) {
      warn(
        loggingLevel,
        'CheckIn: fetchAllRecentMeasurements called with no current user ID.'
      );
      return;
    }
    ...
    try {
      ...
      info(
        loggingLevel,
        'CheckIn: fetchRecentCustomMeasurements returned:',
        custom
      );
      ...
      info(
        loggingLevel,
        `CheckIn: Calling fetchRecentStandardMeasurements for range ${formattedStartDate} to ${formattedEndDate}...`
      );
      const standard = await fetchRecentStandardMeasurements(
        formattedStartDate,
        formattedEndDate
      );
      info(
        loggingLevel,
        'CheckIn: fetchRecentStandardMeasurements returned:',
        standard
      );
      ...
      custom.forEach((m) => {
        ...
        debug(
          loggingLevel,
          `CheckIn: Custom Measurement - Raw entry_date: ${m.entry_date}, entry_timestamp: ${m.entry_timestamp}, Formatted for display: ${formatDateInUserTimezone(m.entry_date, 'PPP')}`
        );
      });
      ...
      info(
        loggingLevel,
        'All recent measurements fetched successfully:',
        allMeasurements.slice(0, 20)
      );
    } catch (err) {
      error(loggingLevel, 'Error fetching all recent measurements:', err);
      sonnerToast.error(
        t(
          'checkIn.failedToLoadRecentMeasurements',
  ...
  const handleDeleteMeasurementClick = async (
    ...
    if (!currentUserId) {
      warn(
        loggingLevel,
        'CheckIn: handleDeleteMeasurementClick called with no current user ID.'
      );
      return;
    }
    ...
    try {
      if (measurement.type === 'custom') {
        ...
        switch (measurement.display_name) {
          ...
          default:
            warn(
              loggingLevel,
              `CheckIn: Unknown standard measurement type for deletion: ${measurement.display_name}`
            );
            return;
        }
        info(
          loggingLevel,
          `CheckIn: Updating check-in measurement field ${fieldToNull} to null for ID: ${standardId}`
        );
      ...
      info(
        loggingLevel,
        'CheckIn: Measurement deleted successfully:',
        measurement.id
      );
      sonnerToast.success(
        t(
      ...
      sonnerToast.error(
        t('checkIn.failedToDeleteMeasurement', 'Failed to delete measurement')
      );
    }
  };
  ...
  const loadExistingData = async () => {
    info(
      loggingLevel,
      `CheckIn: loadExistingData called for date: ${selectedDate}`
    );
    try {
      // Load check-in measurements
      info(
        loggingLevel,
        `CheckIn: Calling loadExistingCheckInMeasurements for date: ${selectedDate}`
      );
      ...
      if (data) {
        ...
        const convertedWeight =
          data.weight !== undefined && data.weight !== null
            ? convertWeight(data.weight, 'kg', defaultWeightUnit)
            : NaN;
        setWeight(
        ...
        const convertedNeck =
          data.neck !== undefined && data.neck !== null
            ? convertMeasurement(data.neck, 'cm', defaultMeasurementUnit)
            : NaN;
        setNeck(
        ...
        const convertedWaist =
          data.waist !== undefined && data.waist !== null
            ? convertMeasurement(data.waist, 'cm', defaultMeasurementUnit)
            : NaN;
        setWaist(
        ...
        const convertedHips =
          data.hips !== undefined && data.hips !== null
            ? convertMeasurement(data.hips, 'cm', defaultMeasurementUnit)
            : NaN;
        setHips(
        ...
        const convertedHeight =
          data.height !== undefined && data.height !== null
            ? convertMeasurement(data.height, 'cm', defaultMeasurementUnit)
            : NaN;
        setHeight(
        ...
        info(
          loggingLevel,
          `CheckIn: No existing check-in data for date ${selectedDate}, clearing form.`
        );
        setWeight('');
        setNeck('');
        setWaist('');
      ...
      info(
        loggingLevel,
        'Fetching mood entry for selectedDate:',
        selectedDate,
        'and currentUserId:',
        currentUserId
      );
      ...
      debug(
        loggingLevel,
        'CheckIn: Mood entry from getMoodEntryByDate:',
        moodEntry
      );
      if (moodEntry) {
        ...
        info(
          loggingLevel,
          `CheckIn: No existing mood entry for date ${selectedDate}, setting to default.`
        );
      ...
      info(
        loggingLevel,
        `CheckIn: Calling loadExistingCustomMeasurements for date: ${selectedDate}`
      );
      const customData = await loadExistingCustomMeasurements(selectedDate);
      info(loggingLevel, 'CheckIn: Custom measurements loaded for date:', {
        selectedDate,
        customData,
      });
      if (customData && customData.length > 0) {
        ...
        info(
          loggingLevel,
          `CheckIn: No existing custom measurements for date ${selectedDate}, clearing form.`
        );
        setCustomValues({});
        setCustomNotes({});
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      info(
        loggingLevel,
        'Attempting to save mood entry with moodToSend:',
        moodToSend,
        'and moodNotes:',
        moodNotes,
        'and selectedDate:',
    ...
    if (!currentUserId) {
      ...
      toast({
        title: t('common.error', 'Error'),
        description: t(
          'checkIn.mustBeLoggedInToSave',
          'You must be logged in to save check-in data'
        ),
        variant: 'destructive',
      });
      return;
    ...
    try {
      ...
      info(
        loggingLevel,
        'CheckIn: Saving standard check-in measurements:',
        measurementData
      );
      ...
      for (const [categoryId, inputValue] of Object.entries(customValues)) {
        ...
        if (!category) {
          warn(
            loggingLevel,
            `CheckIn: Custom category not found for ID: ${categoryId}`
          );
          continue;
        }
        ...
        info(
          loggingLevel,
          `CheckIn: Saving custom measurement for category ${category.name}:`,
          customMeasurementData
        );
        await saveCustomMeasurement(customMeasurementData);
        info(
          loggingLevel,
          `CheckIn: Custom measurement for category ${category.name} saved successfully.`
        );
      }
  ...
  const handleCalculateBodyFat = async () => {
    ...
    try {
      ...
      if (!userProfile) {
        error(
          loggingLevel,
          'CheckIn: Error calculating body fat: userProfile is null or undefined.'
        );
        toast({
          title: t('common.error', 'Error'),
          description: t(
            'checkIn.couldNotLoadUserProfile',
            'Could not load user profile for calculation.'
          ),
        });
        return;
      }
      if (!prefs) {
        error(
          loggingLevel,
          'CheckIn: Error calculating body fat: preferences are null or undefined.'
        );
        toast({
          title: t('common.error', 'Error'),
          description: t(
            'checkIn.couldNotLoadUserPreferences',
            'Could not load user preferences for calculation.'
          ),
        });
        return;
      }
      ...
      if (errorMessage) {
        error(
          loggingLevel,
          `CheckIn: Error calculating body fat: ${errorMessage}`
        );
        toast({
          title: t('common.error', 'Error'),
      ...
      toast({
        title: t('common.error', 'Error'),
        description: `${t('checkIn.failedToCalculateBodyFat', 'Failed to calculate body fat:')} ${err.message || t('checkIn.anUnknownErrorOccurred', 'An unknown error occurred.')}`,
        variant: 'destructive',
      });
    }
  };
