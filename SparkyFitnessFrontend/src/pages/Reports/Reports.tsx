...
import BodyBatteryCard, {
  BODY_BATTERY_METRICS,
} from '@/pages/Reports/BodyBatteryCard';
import RespirationCard, {
  RESPIRATION_METRICS,
} from '@/pages/Reports/RespirationCard';
...
const Reports = () => {
  ...
  const [exerciseEntries, setExerciseEntries] = useState<DailyExerciseEntry[]>(
    []
  ); // New state for exercise entries
  const [exerciseDashboardData, setExerciseDashboardData] =
    useState<ExerciseDashboardData | null>(null); // New state for exercise dashboard data
  const [customCategories, setCustomCategories] = useState<CustomCategory[]>(
    []
  );
  const [customMeasurementsData, setCustomMeasurementsData] = useState<
    Record<string, CustomMeasurementData[]>
  >({});
  ...
  useEffect(() => {
    debug(
      loggingLevel,
      'Reports: Timezone preference changed or component mounted, initializing/re-initializing default date range.'
    );
    ...
    debug(
      loggingLevel,
      'Reports: Inside date re-initialization useEffect - today:',
      today,
      'twoWeeksAgo:',
      twoWeeksAgo
    );
    debug(
      loggingLevel,
      'Reports: Inside date re-initialization useEffect - formatted today:',
      formatDateInUserTimezone(today, 'yyyy-MM-dd'),
      'formatted twoWeeksAgo:',
      formatDateInUserTimezone(twoWeeksAgo, 'yyyy-MM-dd')
    );
    ...
    debug(
      loggingLevel,
      'Reports.tsx - Raw new Date().toISOString():',
  ...
  useEffect(() => {
    ...
    debug(
      loggingLevel,
      'Reports: Parameters for getMoodEntries - activeUserId:',
      activeUserId,
      'startDate:',
      startDate,
      'endDate:',
    ...
    if (user && activeUserId && startDate && endDate) {
      // Only load reports if dates are set
      loadReports();
    } else {
      info(
        loggingLevel,
        'Reports: Skipping initial report load because user, activeUserId, startDate, or endDate is not yet available.'
      );
    }
    ...
    const handleRefresh = () => {
      info(
        loggingLevel,
        'Reports: Received refresh event, triggering data reload.'
      );
      loadReports();
    };
    ...
    t,
  ]); // Added showWeightInKg, showMeasurementsInCm, defaultWeightUnit, defaultMeasurementUnit to dependencies
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      debug(
        loggingLevel,
        'Reports: moodEntriesResult after Promise.allSettled:',
        moodEntriesResult
      );
      ...
      if (moodEntriesResult.status === 'fulfilled') {
        debug(
          loggingLevel,
          'Reports: moodEntriesResult fulfilled with value:',
          moodEntriesResult.value
        );
      } else {
        error(
          loggingLevel,
          'Reports: moodEntriesResult rejected with reason:',
      ...
      if (reportsDataResult.status === 'fulfilled') {
        setNutritionData(
          reportsDataResult.value.nutritionData as ExtendedNutritionData[]
        );
        setTabularData(
          reportsDataResult.value.tabularData as ExtendedDailyFoodEntry[]
        );
        setExerciseEntries(reportsDataResult.value.exerciseEntries);
        // Apply unit conversions to fetchedMeasurementData
        const measurementDataFormatted =
          reportsDataResult.value.measurementData.map((m) => ({
            entry_date: m.entry_date,
            weight: m.weight
              ? convertWeight(m.weight, 'kg', defaultWeightUnit)
              : undefined,
            neck: m.neck
              ? convertMeasurement(m.neck, 'cm', defaultMeasurementUnit)
              : undefined,
            waist: m.waist
              ? convertMeasurement(m.waist, 'cm', defaultMeasurementUnit)
              : undefined,
            hips: m.hips
              ? convertMeasurement(m.hips, 'cm', defaultMeasurementUnit)
              : undefined,
            steps: m.steps || undefined,
            height: m.height
              ? convertMeasurement(m.height, 'cm', defaultMeasurementUnit)
              : undefined,
        ...
        setCustomMeasurementsData(
          reportsDataResult.value.customMeasurementsData
        );
      } else {
        error(
          loggingLevel,
          'Reports: Failed to load core reports data:',
          reportsDataResult.reason
        );
        setNutritionData([]);
        setTabularData([]);
        setExerciseEntries([]);
      ...
      if (exerciseDashboardResult.status === 'fulfilled') {
        ...
        error(
          loggingLevel,
          'Reports: Failed to load exercise dashboard data:',
          exerciseDashboardResult.reason
        );
      ...
      if (rawStressDataResult.status === 'fulfilled') {
        ...
        error(
          loggingLevel,
          'Reports: Failed to load raw stress data:',
          rawStressDataResult.reason
        );
        setRawStressData([]);
      }
      ...
      if (moodEntriesResult.status === 'fulfilled') {
        ...
        error(
          loggingLevel,
          'Reports: Failed to load mood entries:',
          moodEntriesResult.reason
        );
      ...
      if (fastingDataResult.status === 'fulfilled') {
        ...
        error(
          loggingLevel,
          'Reports: Failed to load fasting data:',
  ...
  const exportFoodDiary = async () => {
    ...
    try {
      ...
      const csvHeaders = [
        t('reports.foodDiaryExportHeaders.date', 'Date'),
        t('reports.foodDiaryExportHeaders.meal', 'Meal'),
        t('reports.foodDiaryExportHeaders.food', 'Food'),
        t('reports.foodDiaryExportHeaders.brand', 'Brand'),
        t('reports.foodDiaryExportHeaders.quantity', 'Quantity'),
        t('reports.foodDiaryExportHeaders.unit', 'Unit'),
        t('reports.foodDiaryExportHeaders.calories', 'Calories ({{unit}})', {
          unit: getEnergyUnitString(energyUnit),
        }),
        t('reports.foodDiaryExportHeaders.protein', 'Protein (g)'),
        t('reports.foodDiaryExportHeaders.carbs', 'Carbs (g)'),
        t('reports.foodDiaryExportHeaders.fat', 'Fat (g)'),
        t('reports.foodDiaryExportHeaders.saturatedFat', 'Saturated Fat (g)'),
        t(
        ...
        t('reports.foodDiaryExportHeaders.transFat', 'Trans Fat (g)'),
        t('reports.foodDiaryExportHeaders.cholesterol', 'Cholesterol (mg)'),
        t('reports.foodDiaryExportHeaders.sodium', 'Sodium (mg)'),
        t('reports.foodDiaryExportHeaders.potassium', 'Potassium (mg)'),
        t('reports.foodDiaryExportHeaders.dietaryFiber', 'Dietary Fiber (g)'),
        t('reports.foodDiaryExportHeaders.sugars', 'Sugars (g)'),
        t('reports.foodDiaryExportHeaders.vitaminA', 'Vitamin A (μg)'),
        t('reports.foodDiaryExportHeaders.vitaminC', 'Vitamin C (mg)'),
        t('reports.foodDiaryExportHeaders.calcium', 'Calcium (mg)'),
        t('reports.foodDiaryExportHeaders.iron', 'Iron (mg)'),
        ...customNutrients.map(
  ...
  const exportExerciseEntries = async () => {
    ...
    try {
      ...
      const csvHeaders = [
        t('reports.exerciseEntriesExportHeaders.date', 'Date'),
        t('reports.exerciseEntriesExportHeaders.exerciseName', 'Exercise Name'),
        t(
        ...
        t('reports.exerciseEntriesExportHeaders.sets', 'Sets'),
        t('reports.exerciseEntriesExportHeaders.reps', 'Reps'),
        t('reports.exerciseEntriesExportHeaders.weight', 'Weight'),
        t('reports.exerciseEntriesExportHeaders.notes', 'Notes'),
        t('reports.exerciseEntriesExportHeaders.category', 'Category'),
        t('reports.exerciseEntriesExportHeaders.equipment', 'Equipment'),
        t(
  ...
  if (!user || !activeUserId) {
    info(
      loggingLevel,
      'Reports: User not signed in, displaying sign-in message.'
    );
    return (
      <div>{t('reports.signInMessage', 'Please sign in to view reports.')}</div>
    );
  }
                                  ...
                                  >
                                    <LineChart data={chartData}>
                                      ...
                                      <YAxis
                                        type="number"
                                        domain={
                                          getCustomYAxisDomain(chartData) ||
                                          undefined
                                        }
                                        tickFormatter={(value) => {
                                          if (
