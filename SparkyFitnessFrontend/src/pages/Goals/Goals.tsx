...
} from '@/services/weeklyGoalPlanService';
import {
  PlusCircle,
  Edit,
  Trash2,
...
const GoalsSettings = () => {
  ...
  const {
    ...
    convertEnergy,
    getEnergyUnitString,
  } = usePreferences(); // Corrected destructuring
  ...
  const [presetMacroInputType, setPresetMacroInputType] = useState<
    'grams' | 'percentages'
  >('grams');
  ...
  const memoizedPresetPercentages = useMemo(
    ...
    [
      currentPreset?.breakfast_percentage,
      currentPreset?.lunch_percentage,
      currentPreset?.dinner_percentage,
      currentPreset?.snacks_percentage,
    ]
  );

  const handlePresetPercentagesChange = useCallback(
    (newPercentages: {
      breakfast: number;
      lunch: number;
      dinner: number;
  ...
  const loadGoalPresets = async () => {
    try {
      ...
      toast({
        ...
        description: t(
          'goals.goalsSettings.errorLoadingGoalPresets',
          'Failed to load goal presets.'
        ),
        variant: 'destructive',
      });
    }
  ...
  const handleSavePreset = async () => {
    ...
    try {
      ...
      if (
        ...
        presetToSave.protein_percentage !== null &&
        presetToSave.carbs_percentage !== null &&
        presetToSave.fat_percentage !== null
      ) {
        // presetToSave.calories is already in kcal
        const protein_grams =
          (presetToSave.calories * (presetToSave.protein_percentage / 100)) / 4;
        const carbs_grams =
          (presetToSave.calories * (presetToSave.carbs_percentage / 100)) / 4;
        const fat_grams =
          (presetToSave.calories * (presetToSave.fat_percentage / 100)) / 9;
        presetToSave = {
          ...presetToSave,
  ...
  const loadWeeklyPlans = async () => {
    try {
      ...
      toast({
        title: t('goals.goalsSettings.error', 'Error'),
        description: t(
          'goals.goalsSettings.errorLoadingWeeklyPlans',
  ...
  if (!user) {
    return (
      <div>
        {t(
          'goals.goalsSettings.pleaseSignIn',
          'Please sign in to manage your goals.'
        )}
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div>{t('goals.goalsSettings.loadingGoals', 'Loading goals...')}</div>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {visibleNutrients.includes('protein') && (
              <div>
                <Label htmlFor="protein">
                  {t('goals.goalsSettings.protein', 'Protein (g)')}
                </Label>
                <Input
                  id="protein"
                  type="number"
            ...
            {visibleNutrients.includes('carbs') && (
              <div>
                <Label htmlFor="carbs">
                  {t('goals.goalsSettings.carbohydrates', 'Carbohydrates (g)')}
                </Label>
                <Input
                  id="carbs"
                  type="number"
            ...
            {visibleNutrients.includes('fat') && (
              <div>
                <Label htmlFor="fat">
                  {t('goals.goalsSettings.fat', 'Fat (g)')}
                </Label>
                <Input
                  id="fat"
                  type="number"
            ...
            {visibleNutrients.includes('saturated_fat') && (
              <div>
                <Label htmlFor="saturated_fat">
                  {t('goals.goalsSettings.saturatedFat', 'Saturated Fat (g)')}
                </Label>
                <Input
                  id="saturated_fat"
                  type="number"
            ...
            {visibleNutrients.includes('polyunsaturated_fat') && (
              <div>
                <Label htmlFor="polyunsaturated_fat">
                  {t(
                    'goals.goalsSettings.polyunsaturatedFat',
            ...
            {visibleNutrients.includes('monounsaturated_fat') && (
              <div>
                <Label htmlFor="monounsaturated_fat">
                  {t(
                    'goals.goalsSettings.monounsaturatedFat',
            ...
            {visibleNutrients.includes('trans_fat') && (
              <div>
                <Label htmlFor="trans_fat">
                  {t('goals.goalsSettings.transFat', 'Trans Fat (g)')}
                </Label>
                <Input
                  id="trans_fat"
                  type="number"
                  ...
                  onChange={(e) =>
                    setGoals({ ...goals, trans_fat: Number(e.target.value) })
                  }
                />
              </div>
            )}
            ...
            {visibleNutrients.includes('cholesterol') && (
              <div>
                <Label htmlFor="cholesterol">
                  {t('goals.goalsSettings.cholesterol', 'Cholesterol (mg)')}
                </Label>
                <Input
                  id="cholesterol"
                  type="number"
                  ...
                  onChange={(e) =>
                    setGoals({ ...goals, cholesterol: Number(e.target.value) })
                  }
                />
              </div>
            )}
            {visibleNutrients.includes('sodium') && (
              <div>
                <Label htmlFor="sodium">
                  {t('goals.goalsSettings.sodium', 'Sodium (mg)')}
                </Label>
                <Input
                  id="sodium"
                  type="number"
                  value={goals.sodium}
                  onChange={(e) =>
                    setGoals({ ...goals, sodium: Number(e.target.value) })
                  }
                />
              </div>
            )}
            {visibleNutrients.includes('potassium') && (
              <div>
                <Label htmlFor="potassium">
                  {t('goals.goalsSettings.potassium', 'Potassium (mg)')}
                </Label>
                <Input
                  id="potassium"
                  type="number"
                  ...
                  onChange={(e) =>
                    setGoals({ ...goals, potassium: Number(e.target.value) })
                  }
                />
              </div>
            )}
            {visibleNutrients.includes('dietary_fiber') && (
              <div>
                <Label htmlFor="dietary_fiber">
                  {t('goals.goalsSettings.dietaryFiber', 'Dietary Fiber (g)')}
                </Label>
                <Input
                  id="dietary_fiber"
                  type="number"
                  ...
                  onChange={(e) =>
                    setGoals({
                      ...goals,
                      dietary_fiber: Number(e.target.value),
                    })
                  }
                />
              </div>
            ...
            {visibleNutrients.includes('sugars') && (
              <div>
                <Label htmlFor="sugars">
                  {t('goals.goalsSettings.sugars', 'Sugars (g)')}
                </Label>
                <Input
                  id="sugars"
                  type="number"
                  value={goals.sugars}
                  onChange={(e) =>
                    setGoals({ ...goals, sugars: Number(e.target.value) })
                  }
                />
              </div>
            )}
            ...
            {visibleNutrients.includes('vitamin_a') && (
              <div>
                <Label htmlFor="vitamin_a">
                  {t('goals.goalsSettings.vitaminA', 'Vitamin A (mcg)')}
                </Label>
                <Input
                  id="vitamin_a"
                  type="number"
                  ...
                  onChange={(e) =>
                    setGoals({ ...goals, vitamin_a: Number(e.target.value) })
                  }
                />
              </div>
            )}
            {visibleNutrients.includes('vitamin_c') && (
              <div>
                <Label htmlFor="vitamin_c">
                  {t('goals.goalsSettings.vitaminC', 'Vitamin C (mg)')}
                </Label>
                <Input
                  id="vitamin_c"
                  type="number"
                  ...
                  onChange={(e) =>
                    setGoals({ ...goals, vitamin_c: Number(e.target.value) })
                  }
                />
              </div>
            )}
            {visibleNutrients.includes('calcium') && (
              <div>
                <Label htmlFor="calcium">
                  {t('goals.goalsSettings.calcium', 'Calcium (mg)')}
                </Label>
                <Input
                  id="calcium"
                  type="number"
                  value={goals.calcium}
                  onChange={(e) =>
                    setGoals({ ...goals, calcium: Number(e.target.value) })
                  }
                />
              </div>
            )}
            {visibleNutrients.includes('iron') && (
              <div>
                <Label htmlFor="iron">
                  {t('goals.goalsSettings.iron', 'Iron (mg)')}
                </Label>
                <Input
                  id="iron"
                  type="number"
                  value={goals.iron}
                  onChange={(e) =>
                    setGoals({ ...goals, iron: Number(e.target.value) })
                  }
                />
              </div>
            )}
            ...
            <div>
              <Label htmlFor="water">
                {t('goals.goalsSettings.waterGoal', {
                  unit: water_display_unit,
                  defaultValue: 'Water Goal ({{unit}})',
                })}
              </Label>
              <Input
                id="water"
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setGoals({
                    ...goals,
                    target_exercise_calories_burned: Number(e.target.value),
                  })
                }
              />
            </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {t('goals.goalsSettings.presetKcalMacros', {
                        ...
                        protein: Number(preset.protein || 0).toFixed(0),
                        carbs: Number(preset.carbs || 0).toFixed(0),
                        fat: Number(preset.fat || 0).toFixed(0),
      ...
      <Dialog open={isPresetDialogOpen} onOpenChange={setIsPresetDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {currentPreset?.id
                ? t('goals.goalsSettings.editGoalPreset', 'Edit Goal Preset')
                : t(
                    'goals.goalsSettings.createNewGoalPreset',
                    'Create New Goal Preset'
                  )}
            </DialogTitle>
            <DialogDescription>
              {t(
                'goals.goalsSettings.defineReusableGoals',
                'Define a reusable set of nutrition and exercise goals.'
              )}
            </DialogDescription>
          </DialogHeader>
          {currentPreset && (
            <div className="space-y-6 py-4">
              ...
              <h3 className="text-lg font-semibold col-span-full">
                {t('goals.goalsSettings.mainNutrients', 'Main Nutrients')}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    type="number"
                    value={Math.round(
                      convertEnergy(currentPreset.calories, 'kcal', energyUnit)
                    )}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                ...
                <div className="col-span-full flex items-center gap-4">
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem
                        value="percentages"
                        id="macro-percentages"
                      />
                      <Label htmlFor="macro-percentages">
                ...
                <div>
                  <Label htmlFor="protein">
                    {presetMacroInputType === 'grams'
                      ? t('goals.goalsSettings.protein', 'Protein (g)')
                      : t(
                          'goals.goalsSettings.proteinPercentage',
                          'Protein (%)'
                        )}
                  </Label>
                  <Input
                    ...
                    value={
                      presetMacroInputType === 'grams'
                        ...
                        : (currentPreset.protein_percentage ?? '')
                    }
                    onChange={(e) => {
                ...
                <div>
                  <Label htmlFor="carbs">
                    {presetMacroInputType === 'grams'
                      ? t(
                          'goals.goalsSettings.carbohydrates',
                          'Carbohydrates (g)'
                        )
                      : t('goals.goalsSettings.carbsPercentage', 'Carbs (%)')}
                  </Label>
                  <Input
                    id="carbs"
                    type="number"
                    value={
                      presetMacroInputType === 'grams'
                        ? currentPreset.carbs
                        : (currentPreset.carbs_percentage ?? '')
                    }
                    onChange={(e) => {
                      const value = Number(e.target.value);
                      if (presetMacroInputType === 'grams') {
                        ...
                        setCurrentPreset({
                          ...currentPreset,
                          carbs_percentage: value,
                          carbs: calculateMacroGrams(
                            currentPreset.calories,
                            value
                          ),
                        });
                ...
                <div>
                  <Label htmlFor="fat">
                    {presetMacroInputType === 'grams'
                      ? t('goals.goalsSettings.fat', 'Fat (g)')
                      : t('goals.goalsSettings.fatPercentage', 'Fat (%)')}
                  </Label>
                  <Input
                    id="fat"
                    type="number"
                  ...
                  {presetMacroInputType === 'percentages' &&
                    t('goals.goalsSettings.calculatedGrams', {
                      protein: Number(
                        calculateMacroGrams(
                          currentPreset.calories,
                          currentPreset.protein_percentage || 0
                        )
                      ).toFixed(0),
                      carbs: Number(
                        calculateMacroGrams(
                          currentPreset.calories,
                          currentPreset.carbs_percentage || 0
                        )
                      ).toFixed(0),
                      fat: Number(
                        calculateMacroGrams(
                          currentPreset.calories,
                          currentPreset.fat_percentage || 0
                        )
                      ).toFixed(0),
                      defaultValue:
                        'Calculated Grams: Protein {{protein}}g, Carbs {{carbs}}g, Fat {{fat}}g',
                    })}
                </div>
              </div>

              {/* Fat Breakdown Section */}
              <h3 className="text-lg font-semibold col-span-full mt-4">
                {t('goals.goalsSettings.fatBreakdown', 'Fat Breakdown')}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <Label htmlFor="saturated_fat">
                    {t('goals.goalsSettings.satFat', 'Sat Fat (g)')}
                  </Label>
                  <Input
                    id="saturated_fat"
                    type="number"
                    value={currentPreset.saturated_fat}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                        saturated_fat: Number(e.target.value),
                ...
                <div>
                  <Label htmlFor="polyunsaturated_fat">
                    {t('goals.goalsSettings.polyFat', 'Poly Fat (g)')}
                  </Label>
                  <Input
                    id="polyunsaturated_fat"
                    type="number"
                    value={currentPreset.polyunsaturated_fat}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                        polyunsaturated_fat: Number(e.target.value),
                ...
                <div>
                  <Label htmlFor="monounsaturated_fat">
                    {t('goals.goalsSettings.monoFat', 'Mono Fat (g)')}
                  </Label>
                  <Input
                    id="monounsaturated_fat"
                    type="number"
                    value={currentPreset.monounsaturated_fat}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                        monounsaturated_fat: Number(e.target.value),
              ...
              <h3 className="text-lg font-semibold col-span-full mt-4">
                {t('goals.goalsSettings.mineralsOther', 'Minerals & Other')}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <Label htmlFor="cholesterol">
                  ...
                  <Input
                    id="cholesterol"
                    type="number"
                    value={currentPreset.cholesterol}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                ...
                <div>
                  <Label htmlFor="dietary_fiber">
                    {t('goals.goalsSettings.fiber', 'Fiber (g)')}
                  </Label>
                  <Input
                    id="dietary_fiber"
                    type="number"
                    value={currentPreset.dietary_fiber}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                        dietary_fiber: Number(e.target.value),
                ...
                <div>
                  ...
                  <Input
                    id="vitamin_a"
                    type="number"
                    value={currentPreset.vitamin_a}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                        vitamin_a: Number(e.target.value),
                      })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="vitamin_c"
                    type="number"
                    value={currentPreset.vitamin_c}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
                        vitamin_c: Number(e.target.value),
                      })
                    }
                  />
                </div>
              ...
              {/* Exercise Section */}
              <h3 className="text-lg font-semibold col-span-full mt-4">
                {t('goals.goalsSettings.exercise', 'Exercise')}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <Label htmlFor="target_exercise_calories_burned">
                    {t(
                      'goals.goalsSettings.exerciseCalories',
                      `Exercise Calories (${getEnergyUnitString(energyUnit)})`
                    )}
                  </Label>
                  <Input
                    id="target_exercise_calories_burned"
                    ...
                    value={Math.round(
                      convertEnergy(
                        currentPreset.target_exercise_calories_burned,
                        'kcal',
                        energyUnit
                      )
                    )}
                ...
                <div>
                  <Label htmlFor="target_exercise_duration_minutes">
                    {t(
                      'goals.goalsSettings.exerciseDuration',
                      'Ex. Duration (min)'
                    )}
                  </Label>
                  <Input
                    id="target_exercise_duration_minutes"
                    type="number"
                    value={currentPreset.target_exercise_duration_minutes}
                    onChange={(e) =>
                      setCurrentPreset({
                        ...currentPreset,
            ...
            <>
              ...
              <DialogFooter>
                ...
                >
                  {presetSaving
                    ? t('goals.goalsSettings.saving', 'Saving...')
                    : t('goals.goalsSettings.savePreset', 'Save Preset')}
                </Button>
              </DialogFooter>
            </>
          )}
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-2xl font-bold">
            {t(
              'goals.goalsSettings.weeklyGoalPlans',
              'Weekly Goal Plans (WIP)'
            )}
          </CardTitle>
                ...
                >
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => handleDeleteWeeklyPlan(plan.id!)}
                    >
                      <Trash2 className="w-4 h-4" />{' '}
                      {t('goals.goalsSettings.delete', 'Delete')}
                    </Button>
                    ...
                    <SelectTrigger className="col-span-3">
                      <SelectValue
                        placeholder={t('goals.goalsSettings.selectPreset', {
                          ...
                          defaultValue: 'Select {{day}} preset',
                        })}
                      />
                    </SelectTrigger>
                    <SelectContent>
