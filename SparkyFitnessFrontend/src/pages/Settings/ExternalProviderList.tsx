            ...
            >
                  ...
                  provider.garmin_connect_status === 'connected' ? (
                    <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                      ...
                      <p className="text-sm text-muted-foreground mt-2">
                        Your Garmin account is connected. To reconnect with
                        different credentials, disconnect first and add a new
                        provider.
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      <p className="text-sm text-muted-foreground col-span-2">
                        Note: Garmin Connect integration is tested with few
                        metrics only. Ensure your Docker Compose is updated to
                        include Garmin section.
                        <br />
                        Sparky Fitness does not store your Garmin email or
                        password. They are used only during login to obtain
                        secure tokens.
                      </p>
                    </>
                  )}
              ...
              {editData.provider_type === 'fitbit' && (
                <>
                  ...
                  <p className="text-sm text-muted-foreground col-span-2">
                    Fitbit integration uses OAuth2. You will be redirected to
                    Fitbit to authorize access after adding the provider.
                    <br />
                    In your{' '}
                    <a
            ...
            <div className="space-y-3">
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    size="icon"
                                    onClick={() =>
                                      handleManualSyncGarmin(provider.id)
                                    }
                                    disabled={loading}
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    size="icon"
                                    onClick={() =>
                                      handleDisconnectGarmin(provider.id)
                                    }
                                    disabled={loading}
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                size="icon"
                                onClick={() =>
                                  handleDisconnectWithings(provider.id)
                                }
                                disabled={loading}
                                className="ml-2 text-red-500"
                              >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                size="icon"
                                onClick={() =>
                                  handleManualSyncFitbit(provider.id)
                                }
                                disabled={loading}
                                className="ml-2 text-blue-500"
                              >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                size="icon"
                                onClick={() =>
                                  handleDisconnectFitbit(provider.id)
                                }
                                disabled={loading}
                                className="ml-2 text-red-500"
                              >
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              await toggleProviderPublicSharing(
                                provider.id,
                                newState
                              );
                              toast({
                              ...
                              toast({
                                ...
                                description:
                                  err.message ||
                                  'Failed to update provider sharing',
                                variant: 'destructive',
                              });
                            }
                          }}
              ...
              <div>
                <p className="text-sm text-muted-foreground">
                  {getProviderTypes().find(
                    (t) => t.value === provider.provider_type
                  )?.label || provider.provider_type}
                  {(provider.provider_type === 'mealie' ||
                    provider.provider_type === 'tandoor') &&
                    provider.base_url &&
                ...
                {provider.provider_type === 'withings' &&
                  provider.has_token && (
                    <div className="text-sm text-muted-foreground">
                      ...
                      {provider.withings_last_sync_at &&
                        provider.withings_token_expires && <span> | </span>}
                ...
                {provider.provider_type === 'fitbit' && provider.has_token && (
                  <div className="text-sm text-muted-foreground">
                    ...
                    {provider.fitbit_last_sync_at &&
                      provider.fitbit_token_expires && <span> | </span>}
