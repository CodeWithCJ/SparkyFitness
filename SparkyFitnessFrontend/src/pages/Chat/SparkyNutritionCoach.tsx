...
import {
  ...
  clearHistory,
} from '@/services/Chatbot/Chatbot_utils';
import {
  processFoodInput,
  addFoodOption,
  ...
  (
    ...
    useImperativeHandle(ref, () => ({
      ...
      clearHistory: (autoClearPreference: string) =>
        clearHistory(autoClearPreference),
    }));
    ...
    const getTodaysNutrition = async (
      ...
      try {
        ...
        if (goalsError) {
          error(
            userLoggingLevel,
            '❌ [Nutrition Coach] Error loading goals:',
            goalsError
          );
        }
        ...
        if (foodError) {
          error(
            userLoggingLevel,
            '❌ [Nutrition Coach] Error loading food entries:',
            foodError
          );
          return null;
        }
        ...
        try {
          ...
          error(
            userLoggingLevel,
            '❌ [Nutrition Coach] Error loading exercise entries:',
            err
          );
        }

        const exerciseCalories = (exerciseEntries || []).reduce(
...
`;
        ...
        error(
          userLoggingLevel,
          '❌ [Nutrition Coach] Error getting nutrition data:',
          err
        );
        return null;
      }
    ...
    const sparkyAIService = new SparkyAIService(
      energyUnit,
      convertEnergy,
      getEnergyUnitString
    ); // Instantiate SparkyAIService with preferences
    ...
    const handleUserInput = async (
      ...
      try {
        ...
        try {
          ...
          const jsonMatch = aiResponse.content.match(
            /```json\n([\s\S]*?)\n```/
          );
          ...
          error(
            userLoggingLevel,
            '❌ [Nutrition Coach] Failed to parse AI response as JSON:',
            jsonError
          );
          ...
          return {
            action: 'advice',
            response:
              aiResponse.content || 'Sorry, I had trouble understanding that.',
          };
        }
        ...
        const determinedEntryDate = parsedResponse.entryDate
          ? extractDateFromInput(parsedResponse.entryDate)
          : extractDateFromInput(input);
        ...
        switch (parsedResponse.intent) {
          case 'log_food': {
            ...
            if (
              ...
              info(
                userLoggingLevel,
                'Food not found in DB, requesting AI options...'
              );
              ...
              if (foodOptions.length > 0) {
                ...
                error(
                  userLoggingLevel,
                  'Failed to generate food options via AI.'
                );
          ...
          case 'chat':
            ...
            return await processChatInput(
              ...
              userLoggingLevel
            ); // Chat doesn't need entryDate
          default:
            warn(
              userLoggingLevel,
              '⚠️ [Nutrition Coach] Unrecognized AI intent:',
        ...
        error(
          userLoggingLevel,
          '❌ [Nutrition Coach] Error in processUserInput:',
          err
        );
        return {
          action: 'none',
    ...
    const callAIForFoodOptions = async (
      ...
      try {
        ...
        const aiResponse =
          await sparkyAIService.processMessage(userMessageContent);

        if (!aiResponse || !aiResponse.content) {
          error(
            userLoggingLevel,
            '❌ [Nutrition Coach] No content received from AI for food options.'
          );
          return [];
        }
        ...
        try {
          ...
          if (
            foodOptions.every(
              (option) =>
                ...
                (option.saturated_fat === undefined ||
                  typeof option.saturated_fat === 'number') &&
                (option.polyunsaturated_fat === undefined ||
                  typeof option.polyunsaturated_fat === 'number') &&
                (option.monounsaturated_fat === undefined ||
                  typeof option.monounsaturated_fat === 'number') &&
                (option.trans_fat === undefined ||
                  typeof option.trans_fat === 'number') &&
                (option.cholesterol === undefined ||
                  typeof option.cholesterol === 'number') &&
                (option.sodium === undefined ||
                  typeof option.sodium === 'number') &&
                (option.potassium === undefined ||
                  typeof option.potassium === 'number') &&
                (option.dietary_fiber === undefined ||
                  typeof option.dietary_fiber === 'number') &&
                (option.sugars === undefined ||
                  typeof option.sugars === 'number') &&
                (option.vitamin_a === undefined ||
                  typeof option.vitamin_a === 'number') &&
                (option.vitamin_c === undefined ||
                  typeof option.vitamin_c === 'number') &&
                (option.calcium === undefined ||
                  typeof option.calcium === 'number') &&
            ...
            error(
              userLoggingLevel,
              '❌ [Nutrition Coach] Mapped food options failed validation:',
              foodOptions
            );
            return [];
          }
          ...
          error(
            userLoggingLevel,
            '❌ [Nutrition Coach] Failed to parse or map JSON response for food options:',
            jsonParseError,
        ...
        error(
          userLoggingLevel,
          '❌ [Nutrition Coach] Error in callAIForFoodOptions:',
          err
        );
