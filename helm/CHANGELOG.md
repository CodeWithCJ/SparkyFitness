...
- **Ingress and HTTPRoute server-routed paths are now configurable** — `ingress.serverPaths` and `httpRoute.serverPaths` (default `[/api, /uploads]`) decide which prefixes go to the server service rather than the frontend. App evolution that adds new top-level backend paths (e.g. `/sse`, `/ws`, `/oauth`) only needs a values change instead of a chart patch.
