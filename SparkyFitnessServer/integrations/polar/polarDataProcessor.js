...
async function processPolarExercises(userId, createdByUserId, exercises = []) {
    ...
    for (const exercise of exercises) {
        try {
            ...
            if (!startTime) {
                log('warn', `[polarDataProcessor] Skipping exercise with no start-time: ${JSON.stringify(exercise)}`);
                continue;
            }
            ...
            const entryDate = new Date(startTime).toISOString().split('T')[0];
            const durationMinutes = duration ? Math.round(iso8601ToSeconds(duration) / 60) : 0;
            ...
            const exerciseId = getVal(exercise, 'id');
            log('error', `Error processing Polar exercise ${exerciseId} for user ${userId}: ${error.message}`);
        }
    }
}
...
async function processPolarPhysicalInfo(userId, createdByUserId, physicalInfo = []) {
    ...
    for (const info of physicalInfo) {
        ...
        if (Object.keys(measurementsToUpsert).length > 0) {
            ...
            log('info', `Upserted Polar check-in measurements for user ${userId} on ${entryDate}.`);
        }

        // Process other physiological metrics as custom measurements
        const physiologicalMetrics = [
            { name: 'Resting Heart Rate', value: getVal(info, 'resting-heart-rate'), unit: 'bpm', frequency: 'Daily' },
            { name: 'Maximum Heart Rate', value: getVal(info, 'maximum-heart-rate'), unit: 'bpm', frequency: 'Daily' },
            { name: 'VO2 Max', value: getVal(info, 'vo2-max'), unit: 'ml/kg/min', frequency: 'Daily' },
            { name: 'Aerobic Threshold', value: getVal(info, 'aerobic-threshold'), unit: 'bpm', frequency: 'Daily' },
            { name: 'Anaerobic Threshold', value: getVal(info, 'anaerobic-threshold'), unit: 'bpm', frequency: 'Daily' }
        ];
    ...
    for (const activity of activities) {
        ...
        if (calories || activeCalories || steps) {
            ...
            const metrics = [
                ...
                { name: 'Active Calories', value: activeCalories, unit: 'kcal', frequency: 'Daily' },
                { name: 'Daily Calories', value: calories, unit: 'kcal', frequency: 'Daily' }
            ];
...
async function processPolarSleep(userId, createdByUserId, sleepNights = []) {
    ...
    for (const night of sleepNights) {
        try {
            ...
            // Summary stats
            const lightSleepSec = (getVal(night, 'light-non-rem-sleep-duration') || 0) + (getVal(night, 'lighter-non-rem-sleep-duration') || 0);
            ...
            const entry = await sleepRepository.upsertSleepEntry(userId, createdByUserId, sleepEntryData);
 ...
 */
async function processPolarNightlyRecharge(userId, createdByUserId, recharges = []) {
    ...
    for (const recharge of recharges) {
        ...
        const metrics = [
            { name: 'Nightly Recharge Score', value: getVal(recharge, 'nightly-recharge-status'), unit: 'score', frequency: 'Daily' },
            { name: 'ANS Charge', value: getVal(recharge, 'ans-charge'), unit: 'score', frequency: 'Daily' },
            { name: 'Overnight HRV', value: getVal(recharge, 'heart-rate-variability-avg'), unit: 'ms', frequency: 'Daily' },
            { name: 'Overnight RHR', value: getVal(recharge, 'heart-rate-avg'), unit: 'bpm', frequency: 'Daily' },
            { name: 'Breathing Rate', value: getVal(recharge, 'breathing-rate-avg'), unit: 'brpm', frequency: 'Daily' }
        ];
