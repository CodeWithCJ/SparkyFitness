...
const envLogLevel =
  process.env.SPARKY_FITNESS_LOG_LEVEL?.trim().toUpperCase() || 'INFO';
const currentLogLevel =
  LOG_LEVELS[envLogLevel] !== undefined
      ...
      try {
        if (arg instanceof Error) {
          ...
          return {
            ...
            //@ts-ignore
            responseData: arg.response?.data
              ? truncateStrings(arg.response.data, 500)
              : undefined,
          };
        }
        ...
        if (typeof arg === 'string') {
          return arg.length > 2000
            ? arg.substring(0, 2000) + '... [truncated]'
            : arg;
        }
        return arg;
      } catch (err) {
