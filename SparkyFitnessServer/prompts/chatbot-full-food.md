...
- **Serving Units & Mismatch Clarification**:
  ...
  - Before logging, check the matched food's available serving units returned by `lookup_food_nutrition` (under `Available Serving Units`). If the user specifies a count unit (like pancakes, slices, pieces, items) but the matched food ONLY has gram-based (`g`) or volume-based (`ml` / `oz`) serving units in its available serving units: **DO NOT log it directly**. Instead, ask the user for clarification (e.g., how many grams one piece/slice/pancake weighs, or if they would prefer to log it in grams) to prevent logging a count as grams (which calculates incorrect calories).
