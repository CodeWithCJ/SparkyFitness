  ...
  try {
    const result = await client.query(
      `SELECT
         ...
         (SELECT created_by_user_id FROM check_in_measurements WHERE user_id = $1 AND entry_date <= $2 ORDER BY entry_date DESC LIMIT 1) as created_by_user_id,
         (SELECT updated_by_user_id FROM check_in_measurements WHERE user_id = $1 AND entry_date <= $2 ORDER BY entry_date DESC LIMIT 1) as updated_by_user_id
       FROM (SELECT 1) AS visible
       WHERE $2::date <= $3::date`,
      [userId, date, maxVisibleDate]
    );
