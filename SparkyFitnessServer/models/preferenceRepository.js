  ...
  try {
    const result = await client.query(
      `UPDATE user_preferences SET
        ...
        exercise_calorie_earn_back_percentage = COALESCE($23, exercise_calorie_earn_back_percentage),
        updated_at = now()
      WHERE user_id = $24
      RETURNING *`,
      [
        ...
        preferenceData.exercise_calorie_earn_back_percentage,
        userId
      ]
    );
  ...
  try {
    const result = await client.query(
     ...
     ON CONFLICT (user_id) DO UPDATE SET
       ...
       exercise_calorie_earn_back_percentage = COALESCE(EXCLUDED.exercise_calorie_earn_back_percentage, user_preferences.exercise_calorie_earn_back_percentage),
       updated_at = now()
     RETURNING *`,
     [
