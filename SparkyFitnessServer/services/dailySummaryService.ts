...
export async function getDailySummary({ targetUserId, date, includeWater }: DailySummaryOptions) {
  ...
  const [goals, foodEntries, exerciseSessions, waterResult] = await Promise.all([
    ...
    foodEntryService.getFoodEntriesByDate(targetUserId, targetUserId, date),
