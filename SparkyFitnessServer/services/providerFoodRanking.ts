/**
 * The shape provider search results come back in, and the rule that decides
 * which of them a person meant.
 *
 * Lives here rather than beside either caller because both of them need it:
 * `externalFoodSearchService` ranks what it is about to return, and
 * `foodProviderLookupService` ranks again when it picks one hit for the chatbot.
 * The lookup service already imports the search service, so putting the rule in
 * either would have made the pair circular (#2418).
 */
