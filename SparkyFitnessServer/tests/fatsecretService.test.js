...
describe("FatSecret Service Mapping", () => {
  describe("mapFatSecretSearchItem", () => {
    ...
    it("should map metric in parentheses correctly", () => {
      const item = {
        food_name: "Test Food",
        food_description: "Per 1 serving (28g) - Calories: 110kcal | Fat: 2.00g | Carbs: 15.00g | Protein: 7.00g"
      };
      const result = mapFatSecretSearchItem(item);
    ...
    it("should map fractional household units", () => {
      const item = {
        food_name: "Test Food",
        food_description: "Per 1/4 cup - Calories: 110kcal | Fat: 2g | Carbs: 15g | Protein: 7g"
      };
      const result = mapFatSecretSearchItem(item);
    ...
    it("should map whole household units correctly", () => {
      const item = {
        food_name: "Test Food",
        food_description: "Per 1 slice - Calories: 70kcal | Fat: 1g | Carbs: 12g | Protein: 2g"
      };
      const result = mapFatSecretSearchItem(item);
    ...
    it("should map mixed fractions like 1 1/2 cups", () => {
      const item = {
        food_name: "Test Food",
        food_description: "Per 1 1/2 cups - Calories: 150kcal | Fat: 2g | Carbs: 25g | Protein: 5g"
      };
      const result = mapFatSecretSearchItem(item);
  ...
  describe("mapFatSecretFood", () => {
    const { mapFatSecretFood } = require("../integrations/fatsecret/fatsecretService");
    ...
    it("should map detailed food with multiple variants", () => {
      ...
      expect(result.variants.some(v => v.serving_size === 15 && v.serving_unit === "g")).toBe(true);
      expect(result.variants.some(v => v.serving_size === 100 && v.serving_unit === "g")).toBe(true);
      expect(result.variants.some(v => v.serving_size === 6.667 && v.serving_unit === "tbsp")).toBe(true);
    });
