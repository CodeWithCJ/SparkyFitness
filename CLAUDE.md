...
- **Root Stack Navigator** (`@react-navigation/stack`): `Onboarding` (shown when no server config exists) + `Tabs` screen + food entry flow (`FoodSearch`, `FoodForm`, `FoodEntryAdd`, `FoodEntryView`, `FoodScan`) + exercise/workout flow (`ExerciseSearch` (modal), `WorkoutForm`, `ActivityForm`, `WorkoutDetail`) + `Logs` + `Sync` screen. Dynamic initial route: `Onboarding` if no `ServerConfig` exists, otherwise `Tabs`.
- **Tab Navigator** (`@bottom-tabs/react-navigation`): **Dashboard** (default), **Diary**, **Add** (opens `AddSheet` bottom sheet with Food/Workout/Measurement options), **Workouts** (exercise history), **Settings**. Uses `CustomTabBar` — custom tab bar with a floating circular "Add" button raised above the bar. `TAB_BAR_HEIGHT = 56` exported for layout calculations.
...
- **utils/** — `dateUtils.ts` (date formatting/arithmetic), `syncUtils.ts` (sync duration/interval types), `unitConversions.ts` (weight kg/lbs and distance km/miles conversions — server storage is metric), `activityDetails.ts` (parses `ActivityDetailResponse[]` from synced sessions into human-readable summaries for Garmin/Withings providers), `concurrency.ts` (`withTimeout`, `runTasksInBatches`, `createConcurrencyLimiter` — used by health sync services), `workoutSession.ts` (shared workout display helpers: `getWorkoutIcon` with keyword matching for HealthKit exercise types, `getSourceLabel`, `formatDuration`, `getFirstImage`, `getSessionCalories`, `getWorkoutSummary`, `CATEGORY_ICON_MAP`), `imageSource.ts` (image source signature tracking for SafeImage), `rateLimiter.ts` (sliding window `RateLimiter` class for API call rate limiting)
- **constants/** — `meals.ts` (meal type config, icons, labels, time-based defaults), `exercise.ts` (`ACTIVITY_EXERCISE_CATEGORIES`, `WORKOUT_EXERCISE_CATEGORIES`, `resolveExerciseEntryTarget` — determines routing to WorkoutForm vs ActivityForm based on exercise category, `isDistanceExercise`)
...
- **Preset** (`type: 'preset'`): Grouped workout with named exercises and per-exercise sets (weight/reps). Created via `WorkoutFormScreen`.
- **Individual** (`type: 'individual'`): Single exercise activity with duration, optional distance, calories. Created via `ActivityFormScreen`.

**Draft system**: `workoutDraftService` persists in-progress forms to AsyncStorage (`@SessionDraft`). Both `useWorkoutForm` and `useActivityForm` use `useDraftPersistence` (extracted generic hook) for auto-saving on changes (300ms debounce) and app backgrounding. `WorkoutsScreen` checks for active drafts before navigating and prompts resume vs. discard.

**Exercise selection**: `ExerciseSearchScreen` replaces the old `ExercisePicker` bottom sheet. Operates in two modes: `entry` mode (navigates forward to `ActivityForm` or `WorkoutForm` based on `resolveExerciseEntryTarget` — routes strength exercises to WorkoutForm, cardio to ActivityForm) and `returnKey` mode (returns selected exercise to caller via `CommonActions.setParams` + nonce pattern, same as `FoodFormScreen`). Callers can pass `entryTarget` param to override the default routing (e.g., WorkoutsScreen forces `'workout'`).
...
| `GET /api/exercise-entries/by-date?selectedDate={date}` | Exercise entries by date | `exerciseApi` |
