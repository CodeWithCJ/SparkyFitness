...
describe('formatDose', () => {
  it('prefers the schedule dose override, in the medication dose_unit', () => {
    const med = { dose_amount: 1, dose_unit: 'tablet', strength_value: 500, strength_unit: 'mg' };
  ...
  it('falls back to strength when no dose is set', () => {
    expect(formatDose({ strength_value: 10, strength_unit: 'mg' })).toBe('10 mg');
  });
...
describe('formatStrengthPerUnit', () => {
  it('formats strength per dose unit', () => {
    const med = { dose_amount: 2, dose_unit: 'tablet', strength_value: 500, strength_unit: 'mg' };
  ...
  it('suppresses the line when the dose mirrors the strength (web-created rows)', () => {
    const med = { dose_amount: 10, dose_unit: 'mg', strength_value: 10, strength_unit: 'mg' };
    expect(formatStrengthPerUnit(med)).toBeNull();
  });

  it('shows the line when value matches but the unit differs', () => {
    const med = { dose_amount: 10, dose_unit: 'ml', strength_value: 10, strength_unit: 'mg' };
    expect(formatStrengthPerUnit(med)).toBe('10 mg per ml');
  });

  it('returns null when there is no strength', () => {
    expect(formatStrengthPerUnit({ dose_amount: 1, dose_unit: 'tablet' })).toBeNull();
  });

  it('returns null when strength is already the primary dose display', () => {
    expect(formatStrengthPerUnit({ strength_value: 10, strength_unit: 'mg' })).toBeNull();
  });

  it('drops the "per" clause when the dose has no unit', () => {
    const med = { dose_amount: 2, dose_unit: null, strength_value: 500, strength_unit: 'mg' };
...
describe('describeSchedule', () => {
  it('describes a daily schedule with its time', () => {
    expect(describeSchedule({ schedule_type_id: 'daily', time_of_day: '08:00' })).toBe('Daily at 8:00 AM');
  });
  ...
  it('lists specific days sorted Sunday-first', () => {
    expect(
      describeSchedule({ schedule_type_id: 'specific_days', days_of_week: [5, 1, 3], time_of_day: '21:00' }),
    ).toBe('Mon, Wed, Fri at 9:00 PM');
  });

  it('falls back to Weekly when day data is missing', () => {
    expect(describeSchedule({ schedule_type_id: 'weekly', days_of_week: [] })).toBe('Weekly');
  });

  it('describes every-n-days intervals', () => {
    expect(describeSchedule({ schedule_type_id: 'every_n_days', interval_days: 3 })).toBe('Every 3 days');
    expect(describeSchedule({ schedule_type_id: 'every_n_days', interval_days: 1 })).toBe('Daily');
  });

  it('describes monthly schedules', () => {
    expect(describeSchedule({ schedule_type_id: 'monthly', day_of_month: 15 })).toBe('Monthly on day 15');
    expect(describeSchedule({ schedule_type_id: 'monthly' })).toBe('Monthly');
  });

  it('describes cyclic schedules', () => {
    expect(
      describeSchedule({ schedule_type_id: 'cyclic', cycle_on_days: 21, cycle_off_days: 7 }),
    ).toBe('21 days on, 7 days off');
  });

  it('labels PRN as-needed and ignores any time', () => {
    expect(describeSchedule({ schedule_type_id: 'prn', time_of_day: '08:00' })).toBe('As needed');
  });
