...
export function widgetSwiftFiles(): string[] {
  const walk = (dir: string): string[] =>
    fs.readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
      ...
      return entry.name.endsWith('.swift') ? [path.relative(WIDGET_ROOT, full)] : [];
    });
...
function usages(pattern: RegExp): WidgetKeyUsage[] {
  return widgetSwiftFiles().flatMap((file) => {
    ...
    return [...source.matchAll(new RegExp(pattern.source, 'g'))].map((match) => ({
      file,
      key: match[1],
    }));
  });
...
export function galleryKeyUsages(): WidgetKeyUsage[] {
  return usages(/(?:configurationDisplayName|\.description)\(\s*"([^"]+)"\s*\)/);
}
...
export function fallbackMapKeys(): Set<string> | null {
  for (const file of widgetSwiftFiles()) {
    const body = /func fallbackWidgetString[\s\S]*?\n\}/.exec(readWidgetSwift(file));
    if (body) {
      return new Set([...body[0].matchAll(/case\s+"([^"]+)"\s*:/g)].map((match) => match[1]));
    }
  }
  return null;
}
