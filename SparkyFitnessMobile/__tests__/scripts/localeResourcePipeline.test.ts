...
function fixtureRoot(): string {
  ...
  const registry = {
    sourceLocale: 'en', fallbackLocale: 'en', locales: {
      ...
      de: { languageCode: 'de', intlLocale: 'de-DE', displayNameKey: 'settings.language.german', defaultDisplayName: 'Deutsch' },
    },
  };
...
function writeNative(root: string, locale: string, android: string, ios: string): void {
  const androidDir = locale === 'en' ? 'values' : `values-${locale}`;
  fs.mkdirSync(path.join(root, 'targets/android-widget/res', androidDir), { recursive: true });
  fs.writeFileSync(path.join(root, 'targets/android-widget/res', androidDir, 'widget_strings.xml'), android);
  fs.mkdirSync(path.join(root, 'targets/widget', `${locale}.lproj`), { recursive: true });
  fs.writeFileSync(path.join(root, 'targets/widget', `${locale}.lproj`, 'Localizable.strings'), ios);
}
...
describe('registry-driven locale resource pipeline', () => {
  ...
  it('allows partial DE widget resources, reports coverage, and rejects malformed target placeholders', () => {
    const root = fixtureRoot();
    writeNative(root, 'en', '<resources><string name="title">Hello %1$s</string><string name="detail">Detail</string></resources>', '"title" = "Hello %@";\n"detail" = "Detail";');
    writeNative(root, 'pl', '<resources><string name="title">Cześć %1$s</string><string name="detail">Szczegół</string></resources>', '"title" = "Cześć %@";\n"detail" = "Szczegół";');
    writeNative(root, 'de', '<resources><string name="title">Hallo %1$s</string></resources>', '"title" = "Hallo %@";');
    const output = run(nativeValidator, ['--root', root]);
    expect(output).toContain('de: 1/2 (1 missing)');
    fs.writeFileSync(path.join(root, 'targets/android-widget/res/values-de/widget_strings.xml'), '<resources><string name="title">Hallo %1$d</string></resources>');
    expect(() => run(nativeValidator, ['--root', root])).toThrow('Android widget de:title placeholder mismatch');
    fs.writeFileSync(path.join(root, 'targets/android-widget/res/values-de/widget_strings.xml'), '<resources><string name="title">Hallo %1$s</string></resources>');
    fs.writeFileSync(path.join(root, 'targets/widget/de.lproj/Localizable.strings'), '"title" = "Hallo %lld";');
