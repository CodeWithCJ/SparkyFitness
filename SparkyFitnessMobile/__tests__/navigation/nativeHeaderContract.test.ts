...
function failNativeHeaderContract(message: string): never {
  throw new Error(
    [
      ...
      '- Root-stack screens with a screen-owned React header and a real iOS back button must set headerBackTitle in App.tsx so iOS does not inherit a stale or misleading back-button label. Screens that replace the iOS back button with a native Cancel header item do not need headerBackTitle.',
    ...
    if (
      ...
      failNativeHeaderContract(
        [
          ...
          `Root-stack routes with screen-owned back buttons that are missing headerBackTitle in App.tsx: ${formatList(rootHeaderRoutesMissingBackTitle)}.`,
