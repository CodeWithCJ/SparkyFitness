...
- **components/** — UI components organized by feature: dashboard cards (CalorieRingCard, MacroCard, MacroSummaryCard, ExerciseProgressCard, HydrationGauge, StepsBarChart, ProgressRing), chart components (ChartTouchOverlay, WeightLineChart, HealthTrendsPager — PagerView swiping between StepsBarChart and WeightLineChart), diary views (FoodSummary with meal-type grouping and delete, ExerciseSummary, SwipeableFoodRow with delete via ReanimatedSwipeable, ServingAdjustSheet for quick quantity changes), food entry (FoodForm — reusable nutrition input form with FoodFormData interface), workout display (WorkoutCard — session card using shared helpers from `utils/workoutSession.ts`), workout editing (EditableExerciseCard — single exercise with image/name and `RestPeriodChip` for per-exercise rest duration; delegates set rendering to `EditableSetList`, EditableSetList — shared header/add-row container wrapping a list of `EditableSetRow`s, used by both workout editing and activity editing, EditableSetRow — weight+reps inputs with swipe-to-delete via ReanimatedSwipeable, WorkoutEditableExerciseList — container for exercise cards with `mode` prop for add vs detail contexts, RestPeriodChip — compact tappable chip showing `Rest · m:ss` or `Ns` (exports `DEFAULT_REST_SEC = 90` and `formatRest`), RestPeriodSheet — bottom sheet picker for rest duration selection), workout execution (ActiveWorkoutBar — persistent global HUD rendered as a sibling of the root navigator, drives the workout timer UI and exposes `navigationRef` for cross-screen navigation; also exports `useActiveWorkoutBarPadding` for screens that need bottom inset when the bar is visible), navigation (CustomTabBar — themed custom tab bar with floating Add button), shared UI (DateNavigator, BottomSheetPicker, CalendarSheet, CollapsibleSection, SegmentedControl, Icon, ConnectionStatus, AddSheet — uses deferred `schedulePresent` via animation frame to avoid race conditions during dismissal, FormInput, StepperInput — generic +/− stepper with text input used for serving sizes and set weight/reps, StatusView — reusable empty/loading/error states, EmptyDayIllustration — SVG shown when no data, SafeImage — image wrapper with error handling and proxy headers, ScreenErrorBoundary — class-based boundary + `withErrorBoundary` HOC that logs via `LogService`, shows a themed fallback, and offers Retry/Go Back), settings UI (ServerConfig, ServerConfigModal — unified server URL/API key/proxy headers editing, AppearanceSettings, SyncFrequency, HealthDataSync — shows per-metric values with loading states), auth (MfaForm — shared MFA component with TOTP/email OTP support, used by ReauthModal and OnboardingScreen), modals (ReauthModal — session re-authentication with server picker and API key fallback, PrivacyPolicyModal, DevTools), and `ui/` low-level primitives (Button, toastConfig — themed toast variants for react-native-toast-message)
...
`LogService.ts` is the single source of truth for app logs. Prefer `addLog(message, status?, details?)` over `console.*` everywhere in app code (see `feedback_logging`).

- **Unified status type**: `LogStatus = 'DEBUG' | 'INFO' | 'WARNING' | 'ERROR'` (no more separate `LogLevel`; legacy `'SUCCESS'` is migrated to `'INFO'` on read via `migrateLogEntry`).
- **Two independent settings** sharing `LogThreshold = 'all' | 'no_debug' | 'warnings_errors' | 'errors_only'`:
  - **Capture level** (`log_capture_level`) — drops entries below the threshold before they hit storage.
  - **View filter** (`log_view_filter` threshold + `log_view_selected_statuses` chip selection) — drives what `LogScreen` shows. The legacy `log_filter`/`log_level` keys are migrated into these on startup and then deleted.
- **Writes are buffered** (batched flush with AppState-aware draining) to avoid AsyncStorage thrash; `flushLogs()` is safe to call from shutdown paths.
- **Log screen** (`LogScreen.tsx`) provides filter controls (capture level picker, view threshold picker, per-status chips) plus the log list. When adding new log sites, pass a structured `details` array rather than stuffing context into `message` — `LogScreen` renders details as separate lines and `diagnosticReportService` preserves them when sharing.
...
iOS home-screen widgets live under `targets/widget/` (managed by the `@bacons/apple-targets` Expo config plugin — configured in `app.config.ts` / `targets/widget/expo-target.config.js`). Two widgets ship today:

- **Calorie widget** (`widgets.swift`, kind `widget`) — small/medium widget showing calorie balance (consumed, burned, remaining, progress ring).
- **Macro widget** (`macroWidget.swift`, kind `macroWidget`) — protein/carbs/fat/calories consumed.

Data flow: the RN app writes snapshots into the **shared iOS app group** (`Constants.expoConfig.extra.iosAppGroup`, defined in `app.identifiers.js`) via `ExtensionStorage` from `@bacons/apple-targets`. `useWidgetSync(summary)` is mounted on `DashboardScreen` and writes `calorieSnapshot` + `macroSnapshot` whenever the daily summary for *today* changes, then calls `ExtensionStorage.reloadWidget(kind)` to force WidgetKit to redraw. Swift widgets read the same keys via `UserDefaults(suiteName:)` (see `SharedHelpers.swift`).

When changing widget display: update both the Swift provider/view and the TS snapshot shape in `useWidgetSync`. When adding a new widget, register its kind in `index.swift`, bump the reload call in `useWidgetSync`, and re-run `npx expo prebuild -c` to regenerate the Xcode target from `targets/widget/expo-target.config.js`.

App Icons for the main app live under `targets/widget/assets/AppIcon.appiconset/` (colocated with the widget extension since the config plugin owns the iOS asset catalog). Assets there are picked up by prebuild — edit them instead of the generated `ios/` files.
