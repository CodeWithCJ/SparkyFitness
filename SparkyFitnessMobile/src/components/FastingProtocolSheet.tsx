...
import { formatMobileNumber, isMobileRtl, mobileT } from '../localization';
...
function formatInputHours(hours: number): string {
  return formatMobileNumber(hours, {
...
const FastingProtocolSheet = forwardRef<FastingProtocolSheetRef>(
  (_props, ref) => {
    ...
    const [customHours, setCustomHours] = useState<string>(() =>
      formatInputHours(DEFAULT_CUSTOM_HOURS),
    );
    ...
    useImperativeHandle(ref, () => ({
      present: initialPresetId => {
        ...
        setCustomHours(formatInputHours(DEFAULT_CUSTOM_HOURS));
    ...
    const parsedCustomHours = useMemo(
      () => parseDecimalInput(customHours),
      [customHours],
    );

    const adjustCustom = (delta: number) => {
      const base = Number.isNaN(parsedCustomHours)
        ? DEFAULT_CUSTOM_HOURS
        : parsedCustomHours;
      const next = Math.max(
        MIN_CUSTOM_HOURS,
        Math.min(MAX_CUSTOM_HOURS, base + delta),
      );
      setCustomHours(formatInputHours(next));
    };

    const handleCustomChange = (text: string) => {
      if (INTEGER_INPUT_REGEX.test(text)) setCustomHours(text);
    };

    const customValid =
      !isCustom ||
      (!Number.isNaN(parsedCustomHours) &&
        Number.isInteger(parsedCustomHours) &&
        parsedCustomHours >= MIN_CUSTOM_HOURS &&
        parsedCustomHours <= MAX_CUSTOM_HOURS);

    const startLabel = useMemo(
      () =>
        startDate.toLocaleString('ar-SA-u-ca-gregory', {
          weekday: 'short',
          hour: 'numeric',
          minute: '2-digit',
        }),
      ...
      startFast(
        ...
        },
        {
          onSuccess: () => {
            ...
            Toast.show({ type: 'success', text1: mobileT('fasting.started') });
          },
          onError: error => {
            addLog(`Failed to start fast: ${error}`, 'ERROR');
            Toast.show({
              type: 'error',
              text1: mobileT('fasting.startFailed'),
          ...
          {FASTING_PRESETS.map(preset => {
            ...
            return (
              <TouchableOpacity
                ...
                accessibilityLabel={mobileT('fasting.selectProtocol', {
                  protocol: preset.displayName,
                })}
              ...
              >
                <View className="flex-row items-center justify-between">
                  ...
                  {preset.id !== CUSTOM_PRESET_ID && (
                    ...
                    >
                      {formatInputHours(preset.fastingHours)}:
                      {formatInputHours(preset.eatingHours)}
                    </Text>
                  )}
                </View>
                ...
                {selected && preset.id === CUSTOM_PRESET_ID && (
                  <View className="flex-row items-center mt-3">
                    <StepperInput
                      ...
                      inputProps={{
                        accessibilityLabel: mobileT('fasting.customHours'),
                      }}
                    />
                    <Text
