...
import { getToolDisplay } from '../../constants/chat';
...
export default function ToolCallCard({ part }: { part: ToolCallMessagePart }) {
  ...
  const [muted, iconSuccess, iconDanger] = useCSSVariable([
    '--color-text-muted',
    '--color-icon-success',
    '--color-icon-danger',
    ...
    >
      ...
      {expanded && (
        <View className="px-3 pb-3">
          {hasResult ? (
            resultIsString ? (
              <Text className="text-text-secondary text-sm">{part.result as string}</Text>
            ) : (
