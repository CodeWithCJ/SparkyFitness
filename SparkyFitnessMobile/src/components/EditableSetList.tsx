      ...
      {sets.length > 0 && (
        <Animated.View className="mt-2" layout={LinearTransition.duration(300)}>
          <View className="flex-row items-center py-1 mb-1">
            <Text className="text-xs font-semibold text-text-muted w-10 text-center">{t('editableSets.set')}</Text>
            {durationLike ? (
              <Text className="text-xs font-semibold text-text-muted flex-1 text-center">{t('editableSets.seconds')}</Text>
            ) : (
              <>
                {modality !== 'reps_only' && (
                    <Text className="text-xs font-semibold text-text-muted flex-1 text-center">{t('editableSets.weight')}</Text>
                )}
                <Text className="text-xs font-semibold text-text-muted flex-1 text-center">{t('editableSets.reps')}</Text>
              </>
            )}
            <View style={{ width: 18 }} />
          </View>
