...
const CycleIcon: React.FC<CycleIconProps> = ({ id, size = 28 }) => {
  ...
  const [surfaceBg, borderSubtle, textPrimary] = useCSSVariable([
    '--color-surface',
    '--color-border-subtle',
    '--color-text-primary',
  ]) as [string, string, string];

  // Theme-aware tile background & stroke palette
  const tileBg = tokens.surfaceTint || surfaceBg || 'rgba(150, 150, 150, 0.15)';
  const strokeColor = borderSubtle || 'rgba(150, 150, 150, 0.3)';
  ...
  const renderIconContent = () => {
    switch (id) {
      case 'flow-none':
        return (
          <>
            <Rect width="28" height="28" rx="8" fill={tileBg} stroke={strokeColor} strokeWidth="1" />
            <Circle cx="14" cy="14" r="6" fill="none" stroke={tokens.phaseMenstrual} strokeWidth="1.8" />
          </>
        );
      case 'flow-spotting':
        return (
          <>
            <Rect width="28" height="28" rx="8" fill={tileBg} stroke={strokeColor} strokeWidth="1" />
            <Circle cx="14" cy="14" r="4" fill={tokens.phaseMenstrual} />
          </>
        );
      case 'flow-light':
        return (
      ...
      case 'symptom-fatigue':
        return (
          <>
            <Rect width="28" height="28" rx="8" fill={tileBg} stroke={strokeColor} strokeWidth="1" />
            <Path d="M16.5 6.5 a7.5 7.5 0 1 0 5 12.8 a8.6 8.6 0 0 1 -5 -12.8 Z" fill={tokens.categoryAmber} />
          </>
        );
      case 'symptom-nausea':
        return (
          <>
            <Rect width="28" height="28" rx="8" fill={tileBg} stroke={strokeColor} strokeWidth="1" />
            <Path d="M10 8 C10 6.5 12 6.5 12 8 v2.2 C16.8 10.8 20 14 20 17.2 c0 3.4 -2.8 5.3 -6 5.3 c-3.2 0 -6 -1.9 -6 -5.3 c0 -2.6 0.8 -4.6 2 -6 Z" fill={tokens.phaseFollicular} />
          </>
        );
      case 'mood-happy':
        return (
      ...
      case 'product-pad':
        return (
          <>
            <Rect width="28" height="28" rx="8" fill={tileBg} stroke={strokeColor} strokeWidth="1" />
            <Rect x="8.5" y="7" width="11" height="14" rx="5.5" fill={surfaceBg} stroke={tokens.phaseMenstrual} strokeWidth="1.5" />
      ...
      case 'product-tampon':
        return (
          <>
            <Rect width="28" height="28" rx="8" fill={tileBg} stroke={strokeColor} strokeWidth="1" />
            <Rect x="10.5" y="5.5" width="7" height="12.5" rx="3.5" fill={surfaceBg} stroke={tokens.phaseMenstrual} strokeWidth="1.5" />
