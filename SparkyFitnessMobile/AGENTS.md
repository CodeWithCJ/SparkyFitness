...
- `CheckInPhotosSummary` sits under `MeasurementsSummary` in the diary: the day's angles as small thumbnails, a gap where one is missing, and a tap into `ProgressPhotos` on that same date. Both halves are one check-in, keyed on `(user_id, entry_date)` server-side. It carries no weight or delta because the measurements summary directly above already shows them, and it renders nothing on a day with no photos - the diary lists what a day holds, while prompting for what it lacks is the dashboard card's job.
