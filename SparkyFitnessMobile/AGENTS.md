...
- The JS layer (`src/utils/serverUrl.ts`) is the actual security boundary for this rule on both platforms: production allows plain HTTP only to private/LAN/VPN-range hosts (RFC 1918, link-local, unique-local, and local-only TLDs); HTTP to any public host is always rejected. Carrier-grade NAT (Tailscale/ZeroTier's 100.64.0.0/10) is included in that allowance on Android only — on iOS it still requires HTTPS (`IOS_UNVERIFIED_PRIVATE_RANGES`), pending real-device verification of ATS/`NSAllowsLocalNetworking` behavior for that range. Neither platform's OS-level transport config can express "private only" by itself — Android's cleartext permission is now open to any host, and iOS's `NSAllowsLocalNetworking` exempts IP-literal LAN connections plus unqualified/`.local` hostnames, but not a *qualified* mesh-VPN hostname. Use `getInsecureUrlError`/`isInsecureUrlBlocked` (or the stricter `isPasskeyUrlBlocked` for WebAuthn flows, which always require HTTPS) at every new request path — do not add ad hoc scheme checks. Preserve this guard in onboarding, settings, raw fetch paths, and health sync.
