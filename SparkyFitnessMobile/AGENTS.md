...
- Root-stack screens with a screen-owned React header and a real back button must set `headerBackTitle` in `App.tsx` so iOS back labels stay explicit and do not inherit stale labels; close/cancel modal headers do not need it.
