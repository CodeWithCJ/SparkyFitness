...
- The JS layer (`src/utils/serverUrl.ts`) is the actual security boundary for this rule on both platforms: production allows plain HTTP only to private/LAN/VPN-range hosts (RFC 1918, link-local, unique-local, carrier-grade NAT such as Tailscale, and local-only TLDs); HTTP to any public host is always rejected. Neither platform's OS-level transport config can express "private only" by itself — Android's cleartext permission is now open to any host, and iOS's `NSAllowsLocalNetworking` exempts any IP-literal connection (public or private) plus unqualified/`.local` hostnames, but not a *qualified* mesh-VPN hostname. Use `getInsecureUrlError`/`isInsecureUrlBlocked` (or the stricter `isPasskeyUrlBlocked` for WebAuthn flows, which always require HTTPS) at every new request path — do not add ad hoc scheme checks. Preserve this guard in onboarding, settings, raw fetch paths, and health sync.
